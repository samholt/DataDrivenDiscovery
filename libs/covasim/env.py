...
class CovidEnv:
  ...
  def evaluate_simulator_code_wrapper(
    ...
    if env_name == "COVID":
      loss_per_dim_dict = {
          "susceptible": loss_per_dim[0],
          "exposed": loss_per_dim[1],
          "infected": loss_per_dim[2],
          "recovered": loss_per_dim[3],
      }
    return (
        train_loss,
        val_loss,
  ...
  def evaluate_simulator_code_using_pytorch(
    ...
    device = "cuda:0"
    config.run.pytorch_as_optimizer.batch_size = train_data[0].shape[
        0]  # Will need to change for insight
    ...
    def train(model, states_train_batch_i, actions_train_batch_i):
      ...
      for t in range(states_train_batch_i.shape[1]):
        ...
        susceptible, exposed, infected, recovered, total_population = (
            states_train_batch_i[:, t, 0],
            states_train_batch_i[:, t, 1],
            states_train_batch_i[:, t, 2],
            states_train_batch_i[:, t, 3],
            actions_train_batch_i[:, t, 0],
        )
        dx_dt = model(susceptible, exposed, infected, recovered,
                      total_population)
    ...
    if env_name == "COVID-insight":
      ...
      for t in range(19, int_states.shape[1]):
        ...
        dx_dt = model_intervened(susceptible, exposed, infected, recovered,
                                 total_population)
        dx_dt = torch.stack(dx_dt, dim=-1)
      ...
      predicted_int_states = torch.concat((int_states[:, :19, :], pred_states),
                                          dim=1).detach()
      ...
      for t in range(19, int_states.shape[1]):
        ...
        dx_dt = f_model(susceptible, exposed, infected, recovered,
                        total_population)
        dx_dt = torch.stack(dx_dt, dim=-1)
        # pred_state = int_states[:,t] + dx_dt
        pred_state = pred_state + dx_dt
      ...
      predicted_full_states = torch.concat((int_states[:, :19, :], pred_states),
                                           dim=1).detach()

      noint_states = noint_states.detach().cpu().numpy()
      int_states = int_states.detach().cpu().numpy()
      d3_predicted_int_states = predicted_int_states.cpu().numpy()
      d3_predicted_full_states = predicted_full_states.cpu().numpy()

      np.savez(
          "COVID-intervention.npz",
          noint_states=noint_states,
          int_states=int_states,
          d3_predicted_int_states=d3_predicted_int_states,
          d3_predicted_full_states=d3_predicted_full_states,
      )
      ...
      axs[0].plot(
          predicted_int_states[traj_idx, :, 0].cpu(),
          label="D3-intervention",
      )
      # axs[0].set_ylim((0.0, 1.0))
      # axs[0].plot(predicted_full_states[traj_idx,:,0].cpu(), label="D3")
      axs[0].legend()
      axs[1].plot(noint_states[traj_idx, :, 1].cpu(), label="Not Int")
      axs[1].plot(int_states[traj_idx, :, 1].cpu(), label="Int")
      axs[1].plot(
          predicted_int_states[traj_idx, :, 1].cpu(),
          label="D3-intervention",
      )
      # axs[1].plot(predicted_full_states[traj_idx,:,1].cpu(), label="D3")
      axs[1].legend()
      axs[2].plot(noint_states[traj_idx, :, 2].cpu(), label="Not Int")
      axs[2].plot(int_states[traj_idx, :, 2].cpu(), label="Int")
      axs[2].plot(
          predicted_int_states[traj_idx, :, 2].cpu(),
          label="D3-intervention",
      )
      # axs[2].plot(predicted_full_states[traj_idx,:,2].cpu(), label="D3")
      axs[2].legend()
      axs[3].plot(noint_states[traj_idx, :, 3].cpu(), label="Not Int")
      axs[3].plot(int_states[traj_idx, :, 3].cpu(), label="Int")
      axs[3].plot(
          predicted_int_states[traj_idx, :, 3].cpu(),
          label="D3-intervention",
      )
...
def run_simulation_intervention_middle(args, sim_config, verbose=0):
  ...
  params["interventions"] = cv.change_beta(days=intervention_day,
                                           changes=social_distance_beta_change)
...
def simulate(
  ...
  if multi_process:
    for i, result in tqdm(
        enumerate(pool_outer.imap_unordered(sampler, args_in)),
...
def simulate_insight_intervention(
    num_trajectories=20,
    length=60,
    pop_size=1e6,
    num_infected=10000,
  ...
  np.random.randint(0, 2**32 - 1, size=num_trajectories)
  np.random.randint(10000, 100000, size=num_trajectories)
  val_args_in = [
      (seed, infected) for seed, infected in zip(seeds, num_infecteds)
  ]
  ...
  # # # Train
  train_states, train_actions = collect_data(
      args_in, run_simulation_intervention_middle_null, sim_config)
  # # # Val
  # val_states, val_actions = collect_data(val_args_in, run_simulation_intervention_middle_null, sim_config)
  # Intervention
  noint_states, noint_actions = collect_data(
      val_args_in, run_simulation_intervention_middle_null, sim_config)
  # No intervention
  int_states, int_actions = collect_data(val_args_in,
                                         run_simulation_intervention_middle,
                                         sim_config)
...
"""
  ...
  if name == "COVID":
    ...
    train_set, val_set, test_set = simulate_insight_intervention(
        num_trajectories_input, name=name)
    # method = 'D3'
    method = ""
    if method == "D3":
      ...
      class StateDifferential(nn.Module):
        ...
        def forward(
          ...
          d_exposed__dt = (self.beta * susceptible * infected -
                           self.sigma * exposed)
          ...
          residuals = self.residual_mlp(
              torch.stack((susceptible, exposed, infected, recovered), dim=1))
      ...
      (
          ...
          loss_per_dim,
          test_loss,
      ) = gt_env.evaluate_simulator_code_using_pytorch(
          StateDifferential,
          train_set,
          val_set,
          test_set,
  ...
  def test_optim_0_COVID(self):
    if self.name == "COVID-schools-and-social-distancing":
      ...
      def d_state__dt(
          ...
          recovered,
          deceased,
          social_distancing_intervention_active,
          school_closure_intervention_active,
          parameters,
      ):
        # Unpack parameters
        ...
        intervention_school_effectiveness = parameters[
            "intervention_school_effectiveness"]

        # Calculate the effective transmission rate
        effective_beta = (beta * (1 - social_distancing_intervention_active *
                                  intervention_effectiveness) *
                          (1 - school_closure_intervention_active *
                           intervention_school_effectiveness))

        # SEIR model differential equations
        d_susceptible__dt = -effective_beta * susceptible * infected
        d_exposed__dt = (effective_beta * susceptible * infected -
                         sigma * exposed)
      ...
      parameters = {
          ...
          "sigma": (0.2, 0.1),
          "mu": (0.01, 0.005),
          "intervention_effectiveness": (0.5, 0.2),
          "intervention_school_effectiveness": (0.3, 0.1),
      }
      # Logic goes here
      (
          train_loss,
          val_loss,
          optimized_parameters,
      ) = self.env.evaluate_simulator_code_using_jax(
          d_state__dt,
          parameters,
          self.train_data,
          self.val_data,
