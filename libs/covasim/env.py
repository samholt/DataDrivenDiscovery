...
class CovidEnv:
    ...
    def evaluate_simulator_code_using_pytorch(
        ...
        config.run.pytorch_as_optimizer.batch_size = train_data[0].shape[
            0
        ]  # Will need to change for insight
        ...
        if env_name == "COVID-insight":
            ...
            predicted_int_states = torch.concat(
                (int_states[:, :19, :], pred_states), dim=1
            ).detach()
            ...
            predicted_full_states = torch.concat(
                (int_states[:, :19, :], pred_states), dim=1
            ).detach()
...
def run_simulation_intervention_middle(args, sim_config, verbose=0):
    ...
    params["interventions"] = cv.change_beta(
        days=intervention_day, changes=social_distance_beta_change
    )
...
def simulate_insight_intervention(
    ...
    def collect_data(args, fn, sim_config):
        ...
        if multi_process:
            pool_outer = multiprocessing.Pool(
    ...
    # No intervention
    int_states, int_actions = collect_data(
        val_args_in, run_simulation_intervention_middle, sim_config
    )
        ...
        if method == "D3":
            ...
            class StateDifferential(nn.Module):
                ...
                def forward(
                    ...
                    d_exposed__dt = (
                        self.beta * susceptible * infected
                        - self.sigma * exposed
                    )
                    d_infected__dt = (
                        self.sigma * exposed - self.gamma * infected
                    )
                    ...
                    residuals = self.residual_mlp(
                        torch.stack(
                            (susceptible, exposed, infected, recovered), dim=1
                        )
                    )
        ...
        self.config = to_dot_dict(
            {
                "run": {
                    ...
                    "optimization": {"log_optimization": True, "patience": 10},
                }
            }
        )
    ...
    def test_optim_0_COVID(self):
        if self.name == "COVID-schools-and-social-distancing":
            ...
            def d_state__dt(
                ...
                mu = parameters["mu"]
                intervention_effectiveness = parameters[
                ...
                d_exposed__dt = (
                    effective_beta * susceptible * infected - sigma * exposed
                )
                d_infected__dt = (
                    sigma * exposed - gamma * infected - mu * infected
                )
