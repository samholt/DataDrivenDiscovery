...
class CancerEnv:
    ...
    def get_initial_tumor_size(
        self, num_patients=256, env_name="", variation=""
    ):
        ...
        if env_name == "Cancer-ood" or env_name == "Cancer-iid":
            ...
            initial_volumes = (
                np.random.rand(num_patients) * 1149.8817159979046
                + 0.014165261814709135
            )
    ...
    def reset(self, num_patients=1, env_name="", variation=""):
        v = self.get_initial_tumor_size(
            num_patients=num_patients, env_name=env_name, variation=variation
        )
        c = np.zeros_like(v)
        self.state = (v, c)
        self.time_step = 0
    ...
    def state_diff(
        ...
        assert (
            cancer_volume.shape
            == chemo_concentration.shape
            == chemotherapy_dosage.shape
            == radiotherapy_dosage.shape
        ), "Shapes of inputs must be the same"
        assert (
            len(cancer_volume.shape)
            == len(chemo_concentration.shape)
            == len(chemotherapy_dosage.shape)
            == len(radiotherapy_dosage.shape)
            == 1
        ...
        dv_dt = (
            rho * np.log(K / v)
            - beta_c * c
            - (alpha_r * ra + beta_r * np.square(ra))
            + e
        ) * v
        dv_dt[v == 0] = 0
    ...
    def evaluate_simulator_code_on_dataset(
        self, env_state_diff, parameters, data, config={}, logger=None
    ):
        ...
        def f_with_data_inner(x0, data_to_evaluate):
            ...
            for i in range(states_train.shape[1]):
                ...
                dv_dt, dc_dt = env_state_diff(
                    v, c, chemo_dosage, radio_dosage, parameters_in
                )
    ...
    def evaluate_simulator_code(
        ...
        def f_with_data(x0, data_to_evaluate):
            ...
            states_train, actions_train = data_to_evaluate
            assert not np.any(
                np.isnan(states_train)
            ), "States array contains NaN"
            assert not np.any(
                np.isnan(actions_train)
            ), "Actions array contains NaN"
    ...
    def evaluate_simulator_code_using_pytorch_with_neuroevolution(
        ...
        def evaluate_model(
            network: torch.nn.Module, data: Tuple[np.ndarray, np.ndarray]
        ):
            states_train_np, actions_train_np = train_data
        ...
        model_center = min_problem_train_dataset.make_net(
    ...
    def evaluate_simulator_code_using_jax(
        ...
        std_multiplier = 1.0
        x0_stds = (
            jnp.array([parameters[key][1] for key in keys]) * std_multiplier
        )
        ...
        strategy = Strategies[s_name](
            num_dims=len(x0), popsize=1000, elite_ratio=0.5
        )  # 'CMA_ES'

        es_params = strategy.default_params
        es_params = es_params.replace(
            init_min=-3, init_max=3
        )  # , clip_min=1e6, clip_max=15000)
        ...
        x0_optimized_unormalized = jnp.clip(
            x0_optimized_unormalized, a_min=0, a_max=None
        )
        optimized_parameters = array_to_dict(
            x0_optimized_unormalized.tolist(), parameters
        )
        logger.info(
        ...
        loss_per_dim = jnp.mean(
            jnp.square(simulated_states - val_data[0]), axis=(0, 1)
        ).tolist()
...
def simulate(
    ...
    for t in range(length):
        ...
        cancer_diameter_used = calc_diameter(
            np.stack(cancer_queue, axis=1)
        ).mean(1)
        #
        chemo_prob = 1.0 / (
            1.0
        ...
        if (
            ...
            chemo_dosage = (chemo_application_rvs[:, t] < chemo_prob).astype(
                "float"
            ) * env.max_chemo_drug
            radio_dosage = (radio_application_rvs[:, t] < radio_prob).astype(
                "float"
            ) * env.max_radio
        elif env_name == "Cancer-untreated":
...
class TestEnvOptim(unittest.TestCase):
    def setUp(self):
        ...
        env_name = "Cancer"
        self.train_data = simulate(
            self.num_patients, env_name=env_name, variation="train"
        )
        self.val_data = simulate(
            self.num_patients, env_name=env_name, variation="val"
        )
        self.test_data = simulate(
            self.num_patients, env_name=env_name, variation="test"
        )
        self.env = CancerEnv()
