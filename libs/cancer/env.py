...
class CancerEnv:
  ...
  def get_initial_tumor_size(self, num_patients=256, env_name="", variation=""):
    ...
    if env_name == "Cancer-ood" or env_name == "Cancer-iid":
      if variation == "train":
        initial_volumes = (
            np.random.rand(num_patients) * 1149.8817159979046 * 0.5 +
            0.014165261814709135)
      elif variation == "val":
        initial_volumes = (
            np.random.rand(num_patients) * 1149.8817159979046 * 0.5 +
            0.014165261814709135)
        ...
        if env_name == "Cancer-ood":
          initial_volumes = (
              np.random.rand(num_patients) * 1149.8817159979046 * 0.3 +
              1149.8817159979046 * 0.7)
      ...
      initial_volumes = (np.random.rand(num_patients) * 1149.8817159979046 +
                         0.014165261814709135)
    ...
    # Fixed params
    calc_volume(30)  # carrying capacity given in cm, so convert to volume

  def reset(self, num_patients=1, env_name="", variation=""):
    v = self.get_initial_tumor_size(num_patients=num_patients,
                                    env_name=env_name,
                                    variation=variation)
    c = np.zeros_like(v)
    self.state = (v, c)
  ...
  def state_diff(
    ...
    if self.use_e_noise:
      ...
      e = self.e_noise
    assert (cancer_volume.shape == chemo_concentration.shape ==
            chemotherapy_dosage.shape ==
            radiotherapy_dosage.shape), "Shapes of inputs must be the same"
    assert (len(cancer_volume.shape) == len(chemo_concentration.shape) == len(
        chemotherapy_dosage.shape) == len(radiotherapy_dosage.shape) ==
            1), "Inputs must be batched"
    ...
    dc_dt = -c / 2 + ca
    dv_dt = (rho * np.log(K / v) - beta_c * c -
             (alpha_r * ra + beta_r * np.square(ra)) + e) * v
    dv_dt[v == 0] = 0
  ...
  def evaluate_simulator_code_wrapper(
    ...
    if (env_name == "Cancer" or env_name == "Cancer-ood" or
        env_name == "Cancer-iid"):
      loss_per_dim_dict = {
          "tumor_volume_val_loss": loss_per_dim[0],
          "chemo_drug_concentration_val_loss": loss_per_dim[1],
      }
  ...
  def evaluate_simulator_code_on_dataset(self,
                                         env_state_diff,
                                         parameters,
                                         data,
                                         config={},
    ...
    def f_with_data_inner(x0, data_to_evaluate):
      ...
      for i in range(states_train.shape[1]):
        ...
        chemo_dosage, radio_dosage = (
            actions_train[:, i, 0],
            actions_train[:, i, 1],
        )
        # simulated_actions.append(np.stack((chemo_dosage, radio_dosage), axis=1))
        dv_dt, dc_dt = env_state_diff(v, c, chemo_dosage, radio_dosage,
                                      parameters_in)
  ...
  def evaluate_simulator_code(
      self,
      env_state_diff,
      parameters,
      train_data,
      val_data,
      optimizer="OptunaCmaEsSampler",
  ):
    ...
    bounds = [(
        max(
            parameters[key][0] - parameters[key][1] * std_multiplier,
            1e-6,
        ),
        parameters[key][0] + parameters[key][1] * std_multiplier,
    ) for key in keys]
    ...
    if optimizer == "BFGS":
      ...
      def objective(trial):
        ...
        for i, key in enumerate(keys):
          ...
          x0_suggested.append(
              trial.suggest_float(key, bounds[i][0], bounds[i][1]))
    ...
    print(
        f"Optimizer {optimizer} : Final Train MSE: {f_train(x0_optimized)} | Final Val MSE: {f_val(x0_optimized)}"
    )
  ...
  def evaluate_simulator_code_using_pytorch(
    ...
    def compute_eval_loss(model, dataset):
      ...
      with torch.no_grad():
        ...
        for t in range(states.shape[1]):
          ...
          if (env_name == "Cancer" or env_name == "Cancer-ood" or
            ...
            (
                ...
                chemotherapy_drug_concentration,
                chemotherapy_dosage,
            ) = (states[:, t, 0], states[:, t, 1], actions[:, t, 0])
            dx_dt = model(
                tumor_volume,
                chemotherapy_drug_concentration,
                chemotherapy_dosage,
        ...
        loss_per_dim = (torch.mean(torch.square(pred_states - states),
                                   dim=(0, 1)).cpu().tolist())
      model.train()
    ...
    if config.run.optimize_params:
      ...
      for epoch in range(config.run.pytorch_as_optimizer.epochs):
        ...
        for iter_i in range(
          ...
          indices = permutation[iter_i * config.run.pytorch_as_optimizer.
                                batch_size:iter_i *
                                config.run.pytorch_as_optimizer.batch_size +
                                config.run.pytorch_as_optimizer.batch_size]
          ...
          cum_loss += train_opt(f_model, states_train_batch,
                                actions_train_batch)
          iters += 1
    ...
    val_loss, loss_per_dim = compute_eval_loss(f_model,
                                               (states_val, actions_val))
  ...
  def evaluate_simulator_code_using_pytorch_with_neuroevolution(
      self, StateDifferential, train_data, val_data, config={}, logger=None):
    import torch
    ...
    def evaluate_model(network: torch.nn.Module, data: Tuple[np.ndarray,
                                                             np.ndarray]):
      states_train_np, actions_train_np = train_data
      states_train, actions_train = torch.tensor(
          states_train_np,
          dtype=dtype_of(network),
          device=device_of(network),
      ), torch.tensor(
          actions_train_np,
          dtype=dtype_of(network),
          device=device_of(network),
    ...
    for i in range(10):
      ...
      searcher.run(50)
      print(
          f"Best Loss {searcher.status['best_eval']} | Mean Loss {searcher.status['mean_eval']} | Best Params {searcher.status['best']}"
      )
    ...
    print(
        f"Validation Loss Best: {evaluate_model_val(model_best)} | Validation Loss Center: {evaluate_model_val(model_center)}"
    )
    print("")
    ...
    for epoch in range(epochs):
      ...
      for iter_i in range(int(permutation.shape[0] / batch_size)):
        indices = permutation[iter_i * batch_size:iter_i * batch_size +
                              batch_size]
        states_train_batch, actions_train_batch = (
            states_train[indices],
            actions_train[indices],
        )
  ...
  def evaluate_simulator_code_using_jax(
      self,
      env_state_diff,
      parameters,
      train_data,
    ...
    strategy = Strategies[s_name](num_dims=len(x0),
                                  popsize=1000,
                                  elite_ratio=0.5)  # 'CMA_ES'

    es_params = strategy.default_params
    es_params = es_params.replace(init_min=-3,
                                  init_max=3)  # , clip_min=1e6, clip_max=15000)
    ...
    for t in range(num_gens):
      ...
      if config.run.optimization.log_optimization:
        logger.info(
            "{} - # Gen: {}|Fitness: {:.3g}|Params: {} | rng: {}".format(
                s_name,
                t + 1,
                state.best_fitness,
                state.best_member,
    ...
    x0_optimized_unormalized = jnp.clip(x0_optimized_unormalized,
                                        a_min=0,
                                        a_max=None)
    optimized_parameters = array_to_dict(x0_optimized_unormalized.tolist(),
                                         parameters)
    logger.info(
        f"Optimizer JAX : Final Train MSE: {f_train(x0_optimized)} | Final Val MSE: {f_val(x0_optimized)}"
    )

    simulated_states = f_with_data_inner(x0_optimized, val_data)
    # Independent losses per dimension
    loss_per_dim = jnp.mean(jnp.square(simulated_states - val_data[0]),
                            axis=(0, 1)).tolist()
...
def simulate(
  ...
  env = CancerEnv()
  v, c = env.reset(num_patients=num_patients,
                   env_name=env_name,
                   variation=variation)
  states = []
  actions = []
  ...
  for t in range(length):
    ...
    #
    chemo_prob = 1.0 / (
        1.0 + np.exp(-chemo_sigmoid_beta *
                     (cancer_diameter_used - chemo_sigmoid_intercept)))
    radio_prob = 1.0 / (
        1.0 + np.exp(-radio_sigmoid_beta *
                     (cancer_diameter_used - radio_sigmoid_intercept)))

    if (env_name == "Cancer" or env_name == "Cancer-ood" or
        env_name == "Cancer-iid"):
      chemo_dosage = (chemo_application_rvs[:, t]
                      < chemo_prob).astype("float") * env.max_chemo_drug
      radio_dosage = (radio_application_rvs[:, t]
                      < radio_prob).astype("float") * env.max_radio
    elif env_name == "Cancer-untreated":
    ...
    dv_dt, dc_dt = env.state_diff(v, c, chemo_dosage, radio_dosage)
    if env_name == "Cancer-ood" or env_name == "Cancer-iid":
      v += dv_dt * (1.0 / 24.0)
      c += dc_dt * (1.0 / 24.0)
    else:
...
"""
  ...
  if env_name == "Cancer-ood" or env_name == "Cancer-iid":
    print(
        f"[{env_name}] Training set distribution:\tstart[{train_set[0][:,0,0].min()}, {train_set[0][:,0,0].max()}]\tend[{train_set[0][:,-1,0].min()}, {train_set[0][:,-1,0].max()}]"
    )
    print(
        f"[{env_name}] Validation set distribution:\tstart[{val_set[0][:,0,0].min()}, {val_set[0][:,0,0].max()}]\tend[{val_set[0][:,-1,0].min()}, {val_set[0][:,-1,0].max()}]"
    )
    print(
        f"[{env_name}] Test set distribution:\t\tstart[{test_set[0][:,0,0].min()}, {test_set[0][:,0,0].max()}]\tend[{test_set[0][:,-1,0].min()}, {test_set[0][:,-1,0].max()}]"
    )
...
class TestEnvOptim(unittest.TestCase):
  ...
  def setUp(self):
    ...
    initialize(config_path="../../config",
               version_base=None)  # Point to your actual config directory
  ...
  def test_latest_with_pytorch_model(self):
    ...
    class StateDifferential(nn.Module):
      ...
      def __init__(self):
        ...
        self.residual_nn = nn.Sequential(nn.Linear(4, 10), nn.ReLU(),
                                         nn.Linear(10, 2))
