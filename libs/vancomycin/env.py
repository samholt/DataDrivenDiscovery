...
def load_data(config={}, seed=0, env_name="", train_ratio=0.7, val_ratio=0.15):
  p_v = "libs/vancomycin/data/MOA_rawpredictions.csv"
  data = pd.read_csv(p_v, compression="gzip")

  # Selecting relevant columns and filtering
  data = data[["ID", "TIME", "DV", "AMT", "RATE", "DUR", "EVID", "CMT", "MDV"]]
  data = data[(data["MDV"] == 0) | (data["EVID"] == 1)]
  # data = data[data['MDV'] == 0] # | (data['EVID'] == 1)]
  # data.sort_values(by=['ID', 'TIME'], inplace=True)
  data.reset_index(drop=True, inplace=True)

  # Initialize lists for states and actions
  subjects = data["ID"].unique()
  states = []
  actions = []

  for subj in subjects:
    subj_data = data[data["ID"] == subj]

    # States: 'DV' (drug concentration)
    # Actions: 'AMT' (amount administered), 'RATE' (rate of administration), 'DUR' (duration of dosing)
    subj_states = subj_data[["DV"]].values
    subj_actions = subj_data[["AMT", "RATE", "DUR"]].values

    states.append(subj_states)
    actions.append(subj_actions)

  # Convert lists to numpy arrays for further processing with ML models
  states = np.stack([np.array(s) for s in states])
  actions = np.stack([np.array(a) for a in actions])

  print("")

  # Padding sequences to have the same length for all patients
  # states_padded = pad_sequences(states, padding='post', dtype='float32')
  # actions_padded = pad_sequences(actions, padding='post', dtype='float32')
  ...
  # Assuming each time step corresponds to a unit time, such as an hour, and is uniform across all patients
  max_time_length = states.shape[1]
  time = np.arange(max_time_length)  # Generate a time vector

  for i in range(num_patients):
    if i == 15:
      # Extract the states and actions for the patient
      patient_states = states[i]
      patient_actions = actions[i]

      # Plotting the drug concentration over time
      plt.plot(
          time,
          patient_states,
          marker="o",
      ...
      # Marking the dosing events - plotting only where dose > 0
      time[patient_actions[:, 0]
           > 0]  # Assuming the first column in actions is AMT
      patient_actions[:, 0][patient_actions[:, 0] > 0]

      # Adding dosing markers to the plot
      # plt.scatter(dosing_times, [patient_states[j] for j in dosing_times], color='red', s=dosing_amounts/np.max(dosing_amounts)*100, alpha=0.5)  # Scaled dot size

  plt.title(
      "Drug Concentration and Dosing Events Over Time for Multiple Patients")
  plt.xlabel("Time (hours)")
  plt.ylabel("Drug Concentration")
  plt.legend()
  plt.grid(True)

  plt.savefig("drug_concentration_and_dosing.png")
  plt.show()
  plt.clf()
  print("")
  ...
  if False:
    ...
    train_data = (
        df.iloc[:int(total_time_steps * train_ratio),
                1:].values[np.newaxis, :, :],
        None,
    )
    val_data = (
        df.iloc[
            int(total_time_steps * train_ratio):int(total_time_steps *
    ...
    test_data = (
        df.iloc[int(total_time_steps * (train_ratio + val_ratio)):,
                1:].values[np.newaxis, :, :],
        None,
    )
  elif env_name == "Dataset-HL":
    ...
    train_data = (
        df.iloc[:int(total_time_steps * train_ratio),
                1:].values[np.newaxis, :, :],
        df.iloc[:int(total_time_steps *
                     train_ratio), :1].values[np.newaxis, :, :],
    )
    val_data = (
        df.iloc[
            int(total_time_steps * train_ratio):int(total_time_steps *
    ...
    test_data = (
        df.iloc[int(total_time_steps * (train_ratio + val_ratio)):,
                1:].values[np.newaxis, :, :],
        df.iloc[int(total_time_steps *
                    (train_ratio + val_ratio)):, :1].values[np.newaxis, :, :],
    )
  else:
