...
def load_data(config={}, seed=0, env_name="", train_ratio=0.7, val_ratio=0.15):
    ...
    data = data[
        ["ID", "TIME", "DV", "AMT", "RATE", "DUR", "EVID", "CMT", "MDV"]
    ]
    ...
    for i in range(num_patients):
        if i == 15:
            ...
            time[
                patient_actions[:, 0] > 0
            ]  # Assuming the first column in actions is AMT
    ...
    if False:
        ...
        train_data = (
            df.iloc[: int(total_time_steps * train_ratio), 1:].values[
                np.newaxis, :, :
            ],
            None,
        )
        val_data = (
            df.iloc[
                int(total_time_steps * train_ratio) : int(
                    total_time_steps * (train_ratio + val_ratio)
                ),
                1:,
            ].values[np.newaxis, :, :],
            None,
        ...
        test_data = (
            df.iloc[
                int(total_time_steps * (train_ratio + val_ratio)) :, 1:
            ].values[np.newaxis, :, :],
            None,
        )
    elif env_name == "Dataset-HL":
