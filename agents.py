...
def initialize_agent(method_name,
                     env,
                     config,
                     rate_limiter,
...
class D3(Agent):
  ...
  def get_llm_response(self, messages, max_tokens=None, n=1, print_=True):
    ...
    if self.replay_llm_responses_path:
      ...
      try:
        ...
        with open(f"{self.folder_path}{self.name}_llm_responses.json",
                  "w") as f:
    ...
    while not has_returned_successfully:
      try:
        ...
        while (self.max_tokens - num_tokens_consumed_by_chat_request(
            messages=messages, functions=self.functions)) < 700:
          messages.pop(3)
        self.re_tries += 1
  ...
  def generate_reflection_competition_for_generation_dict(
    ...
    if (self.env.env_name == "Cancer" or self.env.env_name == "Cancer-ood" or
        self.env.env_name == "Cancer-iid"):
...
"""
  ...
  def generate_reflection_prompt_with_group(
    ...
    generation_dicts = sorted(generation_dicts,
...
"""
  ...
  def _run(self, state=""):
    ...
    self.max_re_tries = 30
    self.re_tries = 0
    response_messages = self.get_llm_response_with_retries(self.messages,
                                                           n=n)  # Code message
    generation_dicts = []
    history_generation = []
    history_best_generation = []
    ...
    for response_message in response_messages:
      ...
      if code_json not in computed_funcs:
        ...
        generation_dict = {
            "function_return_message": deepcopy(function_return_message),
        ...
        generation_dict = deepcopy([
            generation_dict for generation_dict in history_generation
    ...
    for generation_id in range(1, self.config.run.generations):
      ...
      if self.name == "D3" or self.name == "D3-no-memory":
        ...
        response_message = self.get_llm_response_with_retries(
            messages_i, print_=False)  # Reflection message
...
""",
      ...
      response_messages = self.get_llm_response_with_retries(
          messages_i, n=sub_group_programs)  # Code message
      for response_message in response_messages:
        code_json = json.dumps(response_message["function_call"])
        if code_json not in computed_funcs:
          try:
      ...
      # Remove nan's
      generation_dicts = [
          generation_dict for generation_dict in generation_dicts
...
class DyNODEAgent(Agent):
  ...
  def __init__(self, env, config, logger, rate_limiter):
    ...
    states_train = torch.tensor(states_train,
                                dtype=torch.float32,
                                device=self.device)
    if actions_train is not None:
      actions_train = torch.tensor(actions_train,
                                   dtype=torch.float32,
                                   device=self.device)
    ...
    self.f_model = DyNODEModel(self.states_dim, self.action_dim,
  ...
  def _run(self, state=""):
    ...
    with torch.no_grad():
      ...
      test_loss = MSE(pred_states,
                      states_actions_test[:, :, :self.states_dim]).item()
    self.logger.info(
        f"[{self.env.env_name} {self.name} {self.env.seed}][Test Run completed successfully] MSE TEST LOSS {test_loss:.4f}"
    )
  ...
  def train_model(self):
    ...
    states_val, actions_val = self.env.val_data
    states_val = torch.tensor(states_val,
                              dtype=torch.float32,
                              device=self.device)
    if actions_val is not None:
      actions_val = torch.tensor(actions_val,
                                 dtype=torch.float32,
                                 device=self.device)
    ...
    def train(model, states_actions_train_batch_i):
      ...
      for t in range(states_actions_train_batch_i.shape[1]):
        ...
        pred_state = (states_actions_train_batch_i[:, t, :self.states_dim] +
                      dx_dt)
      pred_states = torch.stack(pred_states, dim=1)
      loss = MSE(
          pred_states,
    ...
    for epoch in range(self.config.run.pytorch_as_optimizer.epochs):
      ...
      for iter_i in range(int(permutation.shape[0] / batch_size)):
        indices = permutation[iter_i * batch_size:iter_i * batch_size +
                              batch_size]
      ...
      if epoch % self.config.run.pytorch_as_optimizer.log_interval == 0:
        ...
        with torch.no_grad():
          ...
          pred_states = torch.stack(pred_states, dim=1)
          val_loss = MSE(pred_states,
                         states_actions_val[:, :, :self.states_dim]).item()
        self.f_model.train()
        self.logger.info(
...
class RNNAgent(Agent):
  ...
  def __init__(self, env, config, logger, rate_limiter):
    ...
    self.f_model = RNNModel(self.states_dim, self.action_dim,
                            states_actions_train)
    self.f_model.to(self.device)
  ...
  def process_dataset_to_rnn_format(self, states_actions, window_length=20):
    ...
    windows = torch.stack(windows, dim=1).view(-1, window_length,
                                               states_actions.shape[-1])
  ...
  def train_model(self):
    ...
    optimizer = optim.Adam(self.f_model.parameters(),
                           lr=self.config.run.rnn_learning_rate)
    ...
    for epoch in range(self.config.run.pytorch_as_optimizer.epochs):
      ...
      for iter_i in range(int(permutation.shape[0] / batch_size)):
        ...
        cum_loss += train_opt(self.f_model, windows_train_batch,
                              labels_train_batch)
        iters += 1
      time_taken = time.perf_counter() - t0
      if epoch % self.config.run.pytorch_as_optimizer.log_interval == 0:
...
class TransformerAgent(Agent):
  ...
  def __init__(self, env, config, logger, rate_limiter):
    ...
    self.f_model = TransformerModel(self.states_dim, self.action_dim,
                                    states_actions_train)
    self.f_model.to(self.device)
    self.model_path = f"{self.config.run.dynode_saved_models_folder}/rnn_model_{self.env.env_name}_0_trajs_{self.config.run.trajectories}.pt"
  ...
  def process_dataset_to_transformer_format(self,
                                            states_actions,
                                            window_length=10,
                                            output_window=1):
    # Prepare the windows and labels
  ...
  def train_model(self):
    ...
    (
        windows_train,
        labels_train,
    ) = self.process_dataset_to_transformer_format(states_actions_train,
  ...
  def _run(self, state=""):
    ...
    for t in tqdm(range(states_test.shape[1])):
      ...
      if actions_test is None:
        dx_dt = self.model.predict([*states_test[:, t:t + 1]],
    ...
    if self.env.env_name == "COVID-insight":
      ...
      for t in tqdm(range(19, int_states.shape[1])):
        pred_states.append(pred_state)
        if actions_test is None:
          dx_dt = self.model.predict([*int_states[:, t:t + 1]],
                                     multiple_trajectories=True)
        else:
          dx_dt = self.model.predict(
              [*int_states[:, t:t + 1]],
