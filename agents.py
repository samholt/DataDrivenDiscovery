...
def initialize_agent(
    method_name, env, config, rate_limiter, wandb_project_name=None, logger=None
):
    ...
    elif "D3" in method_name:
        agent = D3(env, config, logger, rate_limiter, method_name=method_name)
...
class Agent:
    ...
    def get_llm_config(self):
        return get_llm_config(
            self.config, self.logger, self.name, self.rate_limiter
        )
...
class D3(Agent):
    ...
    def get_llm_response(self, messages, max_tokens=None, n=1, print_=True):
        ...
        if detect_cycles(tmp_messages):
            ...
            self.logger.info(
                f"[Increasing LLM temperature to {self.message_hash_same_increase_temperature}]"
            )
        else:
            if self.message_hash_same_increase_temperature > 0:
                self.logger.info(
                    f"[Annealing LLM temperature to {self.message_hash_same_increase_temperature}]"
                )
        ...
        if self.replay_llm_responses_path:
            ...
            try:
                ...
                with open(
                    f"{self.folder_path}{self.name}_llm_responses.json", "w"
                ) as f:
        ...
        while not has_returned_successfully:
            try:
                ...
                if self.re_tries > self.max_re_tries:
                    self.logger.warning(
                        f"[WARNING] Maximum re-tries reached: {self.re_tries}/{self.max_re_tries}, exiting run!"
                    )
                    raise ValueError(
    ...
    def generate_reflection_competition_for_generation_dict(
        self, generation_dict
    ):
        if (
            self.env.env_name == "Cancer"
            or self.env.env_name == "Cancer-ood"
            or self.env.env_name == "Cancer-iid"
        ):
        ...
        elif self.env.env_name == "warfarin":
            val_loss_per_dim_str = f"(Where the val loss per dimension is warfarin_concentration val loss: {generation_dict['code_dict']['loss_per_dim_dict']['warfarin_concentration']:.3g})"
        else:
...
"""
    ...
    def generate_reflection_prompt_with_group(
        self,
        generation_dicts,
        history_generation,
        iteration,
        ...
        generation_dicts = sorted(
            generation_dicts, key=lambda d: d["fitness"], reverse=True
        )
        history_best_completions = []

        for idx, past_completion in enumerate(history_best_generation):
            history_best_completions.append(
                f"Iteration {idx}. Best Val Loss: {past_completion['code_dict']['val_loss']}. Model description: {past_completion['code_dict']['model_description']}"
            )
        ...
        for generation_dict in generation_dicts:
            completions.append(
                self.generate_reflection_competition_for_generation_dict(
                    generation_dict
                )
            )
        completions = "\n".join(completions)
        if self.name == "D3" or self.name == "D3-no-memory":
            return f"""
...
"""
    ...
    def _run(self, state=""):
        ...
        initial_prompt = prompts.first_task_prompt(
            env_name=self.env.env_name, generations=self.config.run.generations
        )
        ...
        n = 1
        self.logger.info(
            f"[Running generation 0] {self.name} | {self.env.env_name} | {self.env.seed} | Sampling n={n} keep_top_samples"
        )
        ...
        self.max_re_tries = 30
        self.re_tries = 0
        response_messages = self.get_llm_response_with_retries(
            self.messages, n=n
        )  # Code message
        generation_dicts = []
        history_generation = []
        ...
        for response_message in response_messages:
            ...
            if code_json not in computed_funcs:
                ...
                generation_dict = {
                    "function_return_message": deepcopy(
                        function_return_message
                    ),
                    "iteration": generation_id,
                    "code_dict": deepcopy(code_dict),
                    "has_success": deepcopy(has_success),
                    "fitness": deepcopy(code_dict["val_loss"]),
                    "code_string": code_json,
                }
                generation_dicts.append(generation_dict)
                history_generation.append(generation_dict)
                computed_funcs.add(
                    json.dumps(response_message["function_call"])
                )
                # except Exception as e:
                #     self.logger.info(f'[Invalid code, skipping evaluation of program] {e}')
            else:
                generation_dict = deepcopy(
                    [
                        generation_dict
                        for generation_dict in history_generation
                        if generation_dict["code_string"] == code_json
                    ][0]
                )
        ...
        mean_fitness = np.mean(
            [generation_dict["fitness"] for generation_dict in generation_dicts]
        )
        fitnesses = [
            generation_dict["fitness"] for generation_dict in generation_dicts
        ]
        ...
        self.logger.info(
            f"[{self.name} | {self.env.env_name} | {self.env.seed}][Generation {generation_id}] | Top Fitness: {top_fitness} | Num Programs: {num_programs} | Mean Fitness: {mean_fitness} | Fitnesses: {fitnesses} | Current Gen Val Loss: {generation_dict['fitness']}"
        )
        result = {
            "method": self.name,
            ...
            "fitnesses": fitnesses,
            "current_gen_val_loss": generation_dict["fitness"],
        }
        self.logger.info(
            f"[{self.name} | {self.env.env_name} | {self.env.seed}][GEN RESULT] {result}"
        )
        ...
        for generation_id in range(1, self.config.run.generations):
            generation_dicts = sorted(
                generation_dicts, key=lambda d: d["fitness"]
            )
            num_programs = len(generation_dicts)
            sub_group_programs = n
            reflection_prompt = self.generate_reflection_prompt_with_group(
                generation_dicts=generation_dicts,
                history_generation=history_generation,
                iteration=generation_id,
                history_best_generation=history_best_generation,
            )
            ...
            if self.name == "D3" or self.name == "D3-no-memory":
                ...
                response_message = self.get_llm_response_with_retries(
                    messages_i, print_=False
                )  # Reflection message
...
""",
                ...
                messages_i = [
                    ...
                    {
                        "role": "user",
                        "content": prompts.first_task_prompt(
                            env_name=self.env.env_name,
                            generations=self.config.run.generations,
                            current_iteration=generation_id,
                        ),
                        "function_call": {
                            "name": "complete_StateDifferential_code"
                        },
            ...
            response_messages = self.get_llm_response_with_retries(
                messages_i, n=sub_group_programs
            )  # Code message
            for response_message in response_messages:
                code_json = json.dumps(response_message["function_call"])
                if code_json not in computed_funcs:
                    try:
                        ...
                        # self.logger.info(f'[Invalid code, skipping evaluation of program] {e}')
                        self.logger.info(
                            f"[Invalid code, attempting to fix the code] {e}"
                        )
            ...
            generation_dicts = [
                generation_dict
                for generation_dict in generation_dicts
                if not np.isnan(generation_dict["fitness"])
            ]
            # Cull poor fitnesses
            generation_dicts = generation_dicts[
                : self.config.run.keep_top_samples
            ]
            mean_fitness = np.mean(
                [
                    generation_dict["fitness"]
                    for generation_dict in generation_dicts
                ]
            )
            fitnesses = [
            ...
            if patience_counter >= self.config.run.d3_patience:
                self.logger.info(
                    f"Early stopping triggered at generation {generation_id}"
                )
        ...
        self.logger.info(
            f"[{self.env.env_name} {self.name} {self.env.seed}][Test Run completed successfully] MSE TEST LOSS {test_loss:.4f}"
        )
        return test_loss
...
class ZeroShot(D3):
    def __init__(
        self, env, config, logger, rate_limiter, optimize_params=False, name=""
    ):
        super().__init__(env, config, logger, rate_limiter)
        self.name = "ZeroZhot"
        self.folder_path = f"{self.config.run.log_path.split('.txt')[0]}/{self.env.env_name}/{self.env.seed}/"
...
class DyNODEAgent(Agent):
    def __init__(self, env, config, logger, rate_limiter):
        ...
        states_train = torch.tensor(
            states_train, dtype=torch.float32, device=self.device
        )
        if actions_train is not None:
            actions_train = torch.tensor(
                actions_train, dtype=torch.float32, device=self.device
            )
            states_actions_train = torch.cat(
                [states_train, actions_train], dim=-1
            )
        else:
            states_actions_train = states_train

        self.f_model = DyNODEModel(
            self.states_dim, self.action_dim, states_actions_train
        )
    ...
    def _run(self, state=""):
        if self.config.run.dynode_retrain_model:
            ...
            if Path(self.model_path).exists():
                ...
                self.logger.info(
                    f"[WARNING] No saved model found at {self.model_path}, training from scratch..."
                )
                self.train_model()

        states_test, actions_test = self.env.test_data
        states_test = torch.tensor(
            states_test, dtype=torch.float32, device=self.device
        )
        if actions_test is not None:
            actions_test = torch.tensor(
                actions_test, dtype=torch.float32, device=self.device
            )
        ...
        with torch.no_grad():
            ...
            for t in range(states_actions_test.shape[1]):
                ...
                pred_state = (
                    states_actions_test[:, t, : self.states_dim] + dx_dt
                )
            pred_states = torch.stack(pred_states, dim=1)
            test_loss = MSE(
                pred_states, states_actions_test[:, :, : self.states_dim]
            ).item()
        self.logger.info(
            f"[{self.env.env_name} {self.name} {self.env.seed}][Test Run completed successfully] MSE TEST LOSS {test_loss:.4f}"
        )
        ...
        if self.env.env_name == "COVID-insight":
            ...
            with torch.no_grad():
                ...
                predicted_dynode_states = torch.concat(
                    (
                        states_actions_test[:, :19, : self.states_dim],
                        pred_states,
                    ),
                    dim=1,
                ).detach()
                ...
                axs[0].plot(
                    predicted_dynode_states[traj_idx, :, 0], label="DYNODE"
                )
                # axs[0].set_ylim((0.0, 1.0))
                # axs[0].plot(predicted_full_states[traj_idx,:,0], label="D3")
                axs[0].legend()
                axs[1].plot(noint_states[traj_idx, :, 1], label="Not Int")
                axs[1].plot(int_states[traj_idx, :, 1], label="Int")
                axs[1].plot(
                    predicted_dynode_states[traj_idx, :, 1], label="DYNODE"
                )
                # axs[1].plot(predicted_full_states[traj_idx,:,1], label="D3")
                axs[1].legend()
                axs[2].plot(noint_states[traj_idx, :, 2], label="Not Int")
                axs[2].plot(int_states[traj_idx, :, 2], label="Int")
                axs[2].plot(
                    predicted_dynode_states[traj_idx, :, 2], label="DYNODE"
                )
                # axs[2].plot(predicted_full_states[traj_idx,:,2], label="D3")
                axs[2].legend()
                axs[3].plot(noint_states[traj_idx, :, 3], label="Not Int")
                axs[3].plot(int_states[traj_idx, :, 3], label="Int")
                axs[3].plot(
                    predicted_dynode_states[traj_idx, :, 3], label="DYNODE"
                )
                # axs[3].plot(predicted_full_states[traj_idx,:,3].cpu(), label="D3")
                axs[3].legend()
    ...
    def train_model(self):
        ...
        states_val = torch.tensor(
            states_val, dtype=torch.float32, device=self.device
        )
        if actions_val is not None:
            actions_val = torch.tensor(
                actions_val, dtype=torch.float32, device=self.device
            )
        ...
        batch_size = min(
            self.config.run.pytorch_as_optimizer.batch_size,
            states_actions_train.shape[0],
        )
        MSE = torch.nn.MSELoss()
        optimizer = optim.Adam(
            self.f_model.parameters(),
            lr=self.config.run.dynode_learning_rate,
            weight_decay=self.config.run.pytorch_as_optimizer.weight_decay,
        )
        ...
        def train(model, states_actions_train_batch_i):
            ...
            for t in range(states_actions_train_batch_i.shape[1]):
                ...
                pred_state = (
                    states_actions_train_batch_i[:, t, : self.states_dim]
                    + dx_dt
                )
            pred_states = torch.stack(pred_states, dim=1)
            loss = MSE(
                pred_states,
                states_actions_train_batch_i[:, :, : self.states_dim],
            )
            loss.backward()
            # if clip_grad_norm:
        ...
        for epoch in range(self.config.run.pytorch_as_optimizer.epochs):
            ...
            permutation = torch.randperm(states_actions_train.shape[0])
            for iter_i in range(int(permutation.shape[0] / batch_size)):
                indices = permutation[
                    iter_i * batch_size : iter_i * batch_size + batch_size
                ]
            ...
            if epoch % self.config.run.pytorch_as_optimizer.log_interval == 0:
                ...
                with torch.no_grad():
                    ...
                    for t in range(states_actions_val.shape[1]):
                        ...
                        pred_state = (
                            states_actions_val[:, t, : self.states_dim] + dx_dt
                        )
                    pred_states = torch.stack(pred_states, dim=1)
                    val_loss = MSE(
                        pred_states, states_actions_val[:, :, : self.states_dim]
                    ).item()
                self.f_model.train()
                self.logger.info(
                    f"[{self.name}][EPOCH {epoch} COMPLETE] MSE TRAIN LOSS {cum_loss/iters:.4f} | MSE VAL LOSS {val_loss:.4f} | s/epoch: {time_taken:.2f}s"
                )
                # Early stopping check
        ...
        ensure_directory_exists(Path(self.model_path).parent)
        torch.save(self.f_model.state_dict(), self.model_path)
        self.logger.info(
            f"[Train Run completed successfully] MSE VAL LOSS {val_loss:.4f}"
        )
        self.logger.info("")
...
class RNNAgent(Agent):
    ...
    def _run(self, state=""):
        ...
        windows_test, labels_test = self.process_dataset_to_rnn_format(
            states_actions_test
        )
    ...
    def process_dataset_to_rnn_format(self, states_actions, window_length=20):
        ...
        for i in range(states_actions.shape[1] - window_length):
            ...
            labels.append(
                states_actions[:, i + window_length, : self.states_dim]
            )

        # Convert lists to tensors
        windows = torch.stack(windows, dim=1).view(
            -1, window_length, states_actions.shape[-1]
        )
    ...
    def train_model(self):
        ...
        windows_train, labels_train = self.process_dataset_to_rnn_format(
            states_actions_train
        )
        windows_val, labels_val = self.process_dataset_to_rnn_format(
            states_actions_val
        )
        ...
        optimizer = optim.Adam(
            self.f_model.parameters(), lr=self.config.run.rnn_learning_rate
        )
        ...
        for epoch in range(self.config.run.pytorch_as_optimizer.epochs):
            ...
            for iter_i in range(int(permutation.shape[0] / batch_size)):
                ...
                windows_train_batch, labels_train_batch = (
                    windows_train[indices],
                    labels_train[indices],
                )
                cum_loss += train_opt(
                    self.f_model, windows_train_batch, labels_train_batch
                )
                iters += 1
            time_taken = time.perf_counter() - t0
            if epoch % self.config.run.pytorch_as_optimizer.log_interval == 0:
...
class TransformerAgent(Agent):
    ...
    def _run(self, state=""):
        ...
        if self.env.env_name == "warfarin":
            window_length = 2
        else:
            window_length = 10
        windows_test, labels_test = self.process_dataset_to_transformer_format(
            states_actions_test, window_length=window_length
        )
    ...
    def process_dataset_to_transformer_format(
        self, states_actions, window_length=10, output_window=1
    ):
        # Prepare the windows and labels
        ...
        for i in range(states_actions.shape[1] - window_length):
            ...
            labels.append(
                states_actions[
                    :,
                    i + output_window : i + window_length + output_window,
                    : self.states_dim,
                ]
            )
    ...
    def train_model(self):
        ...
        (
            windows_train,
            labels_train,
        ) = self.process_dataset_to_transformer_format(
            states_actions_train, window_length=window_length
        )
        windows_val, labels_val = self.process_dataset_to_transformer_format(
            states_actions_val, window_length=window_length
        )
    ...
    def _run(self, state=""):
        ...
        for t in tqdm(range(states_test.shape[1])):
            ...
            if actions_test is None:
                dx_dt = self.model.predict(
                    [*states_test[:, t : t + 1]], multiple_trajectories=True
                )
            else:
                dx_dt = self.model.predict(
                    [*states_test[:, t : t + 1]],
                    u=[*actions_test[:, t : t + 1]],
        ...
        test_loss = MSE(
            torch.tensor(pred_states), torch.tensor(states_test)
        ).item()
        ...
        if self.env.env_name == "COVID-insight":
            ...
            for t in tqdm(range(19, int_states.shape[1])):
                ...
                if actions_test is None:
                    dx_dt = self.model.predict(
                        [*int_states[:, t : t + 1]], multiple_trajectories=True
                    )
                else:
                    dx_dt = self.model.predict(
                        [*int_states[:, t : t + 1]],
            ...
            predicted_sindy_states = np.concatenate(
                (int_states[:, :19, :], pred_states), axis=1
            )

            np.savez(
                "COVID-intervention-sindy.npz",
    ...
    def train_model(self):
        ...
        self.model = ps.SINDy(
            optimizer=STLSQ(
                ...
                alpha=self.sindy_alpha,
                max_iter=100,
                ridge_kw={"tol": 1e-6},
            ),
            feature_library=PolynomialLibrary(**self.PolynomialLibrary_kw),
        )
        ...
        if actions_train is None:
            ...
            self.model.fit(
                [*states_train], u=[*actions_train], multiple_trajectories=True
            )
        # self.model.print()
        model_str = return_sindy_string(self.model)
        self.logger.info(
            f"[{self.env.env_name} {self.name} {self.env.seed}][Model] {model_str}"
        )
        ...
        if self.compute_val_loss:
            ...
            self.logger.info(
                f"[{self.name}][COMPLETE] MSE VAL LOSS {val_loss:.4f}"
            )
