...
def run(config: DictConfig) -> None:
  log_path = generate_log_file_path(__file__,
                                    log_folder=config.setup.log_dir,
                                    config=config)
...
def run_exp_wrapper(args, logger, **kwargs):
  ...
  config = dict_to_config(deepcopy(OmegaConf.to_container(config,
                                                          resolve=True)))
...
def run_exp(env_name,
  ...
  result = simulate(env_name, method_name, seed, logger, rate_limiter, config,
                    wandb)
