...
from utils.exp_utils import config_to_dict, dict_to_config, seed_all
from utils.logging_utils import (
    Experiment,
    create_logger_in_process,
...
def run(config: DictConfig) -> None:
    log_path = generate_log_file_path(
        __file__, log_folder=config.setup.log_dir, config=config
    )
    ...
    )  # ChatRateLimiter(request_limit=request_limit, token_limit=token_limit)
    ...
    if config.setup.flush_mode:
        logger.info("[WARNING] In FLUSH MODE -- TEST RUN ONLY")
...
def main(config, wandb, logger, rate_limiter):
    ...
    if experiment == Experiment.MAIN_TABLE:
        ...
        for seed in range(
            ...
            for trajectories in config.setup.trajectories_sweep:
                ...
                for method_name in config.setup.methods_to_evaluate:
                    args_for_runs.append(
                        (env_name, method_name, seed, trajectories)
                    )
    ...
    elif experiment == Experiment.D3_ABLATION_NO_CRITIC:
        env_name = "Cancer"
        method_name = "D3-no-critic"
        for seed in range(
            config.setup.seed_start,
            config.setup.seed_runs + config.setup.seed_start,
        ):
    ...
    elif experiment == Experiment.D3_ABLATION_NO_MEMORY:
        env_name = "Cancer"
        method_name = "D3-no-memory"
        for seed in range(
            config.setup.seed_start,
            config.setup.seed_runs + config.setup.seed_start,
        ):
    ...
    evaluate_policy_single = partial(
        run_exp_wrapper_outer,
        config=config,
        wandb=wandb,
    ...
    if not config.setup.multi_process_results:
        ...
        for i, result in tqdm(
            enumerate(
                pool_outer.imap_unordered(evaluate_policy_single, args_for_runs)
            ),
    ...
    logger.info(
        f"Time taken for all runs: {time_taken}s\t| {time_taken/60.0} minutes"
    )
    if config.setup.multi_process_results:
        pool_outer.close()
...
def run_exp_wrapper(args, logger, **kwargs):
    ...
    config = dict_to_config(
        deepcopy(OmegaConf.to_container(config, resolve=True))
    )
...
def run_exp_wrapper_outer(args, **kwargs):
    ...
    if config.setup.debug_mode:
        ...
        try:
            ...
            print("")
    result.update(
        {"env_name": env_name, "seed": seed, "method_name": method_name}
    )
    return result


def run_exp(
    env_name, method_name, seed, logger, rate_limiter, config={}, wandb=None
):
    logger.info(f"Running {env_name} {method_name} {seed}")
    t00 = time.perf_counter()
    result = simulate(
        env_name, method_name, seed, logger, rate_limiter, config, wandb
    )
    ...
    result.update(
        {
            ...
            "experiment": config.setup.experiment.name,
        }
    )
    return result
