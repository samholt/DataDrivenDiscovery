...
sf = 3
env_name_map = {
    "Cancer": "Lung Cancer (with Chemo. \& Radio.)",
...
method_name_map = {
    ...
    "D3-no-memory": "DI3-no-memory",
    "D3-no-critic": "DI3-no-critic",
}
...
def df_from_log(path, remove_extra_columns=True, load_tensorboard_data=True):
    ...
    if "[Exp evaluation complete] {" in line:
      ...
      if load_tensorboard_data:
        if "run_name" in result_dict:
          ...
          result_dict["episodic_return_all"] = tensorboard_data[
  ...
  if remove_extra_columns:
    ...
    current_columns = list(dfm.columns)
    columns_to_np_arrays = set(columns_to_np_arrays_if_exist) & set(
        current_columns)
...
def normalize_means(df):
  df_means = (df.groupby(["env_name", "policy",
                          "network_specific"]).agg(np.mean).reset_index())
...
def compute_norm_metrics(df):
  ...
  for env_name in df.env_name.unique():
    ...
    df.loc[df.env_name == env_name,
           "test_rmse"] = (df[df.env_name == env_name].test_rmse / norm)
  return df


def generate_main_results(df_results,
                          wandb=None,
                          use_95_ci=True,
                          seeds_to_use=None):
  # Process seeds here
  ...
  df_results = df_results[df_results["seed"] != 10]
  if use_95_ci:
    df_out = (df_results.groupby(["env_name",
                                  "method"]).agg([np.mean, ci]).reset_index())
    error_metric = "ci"
  else:
    df_out = (df_results.groupby(["env_name",
                                  "method"]).agg([np.mean,
  ...
  df_out = df_out.sort_values(
      by=["env_name", "method"],
      key=lambda col: col.map(env_name_ordering)
      if col.name == "env_name" else col.map(method_name_ordering),
  )
  table_lines = []
  ...
  table_lines.append("".join([
      r"&  \multicolumn{1}{c|}{" + env_name_map[env_name] + "}"
      for env_name in df_out.env_name.unique()
  ]) + r"\\")
  table_lines.append(r"Method " +
                     r"& \textbf{Test MSE} " * df_out.env_name.nunique() +
                     r"\\")
  table_lines.append(r"" + r"    & $\downarrow$ " * df_out.env_name.nunique() +
                     r"\\")
  ...
  for method in df_out.method.unique():
    ...
    for env_name in df_out.env_name.unique():
      ...
      if row.empty:
        ...
        line += (r"&\bf" + f"{row.test_mse['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
                 f"{row.test_mse[error_metric].iloc[0]:.{sf}g}")
      else:
        line += (r"&" + f"{row.test_mse['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
                 f"{row.test_mse[error_metric].iloc[0]:.{sf}g}")
    line += r"\\"
    table_lines.append(line)
...
def generate_main_results_ood_table(df_results,
                                    wandb=None,
                                    use_95_ci=True,
                                    seeds_to_use=None):
  # Process seeds here
...
def plot_evolutionary_plots(df_results,
                            wandb=None,
                            use_95_ci=True,
                            seeds_to_use=None):
  # Process seeds here
  ...
  df_results["top_3_mean"] = df_results["fitnesses"].apply(
      lambda x: np.mean(x[:3]))
  df_results["generation_mean"] = df_results["fitnesses"].apply(
      lambda x: np.mean(x))
  # drop fitnesses
  ...
  df_filtered = df_results[df_results["seed"] == 15]
  ...
  for i in range(len(df_filtered)):
    if i == 0:
      ...
      ne = df_filtered.iloc[i]["fitnesses"][0]
      new_elements.append(ne)
    else:
      # Compute the difference between the current and previous list
      new_element_i = list(
          set(df_filtered.iloc[i]["fitnesses"]) -
          set(df_filtered.iloc[i - 1]["fitnesses"]))
      if len(new_element_i) != 0:
        ni = new_element_i[0]
        ne = min(ne, ni)
        new_elements.append(ne)
      else:
...
def generate_main_results_less_samples(df_results,
                                       wandb=None,
                                       use_95_ci=True,
                                       seeds_to_use=None):
  # Process seeds here
  df_results = remove_unneeded_columns(df_results)
  if use_95_ci:
    df_out = (df_results.groupby(["env_name", "method",
                                  "trajectories"]).agg([np.mean,
                                                        ci]).reset_index())
    error_metric = "ci"
  else:
    df_out = (df_results.groupby(["env_name", "method",
  ...
  df_out = df_out.sort_values(
      by=["env_name", "method"],
      key=lambda col: col.map(env_name_ordering)
      if col.name == "env_name" else col.map(plot_method_name_ordering),
  )
...
def generate_main_results_table_paper_format_tests_pass_feature_implement(
    df_results,
    wandb=None,
    use_95_ci=True,
    seeds_to_use=None,
    ...
    rebuttal=False,
    rebuttal_big=False,
    ablation_no_summarization=False,
):
  # Process seeds here
  df_results = remove_unneeded_columns(df_results)
  df_results = df_results[df_results["locs"] != 0]
  df_results["features"] = np.minimum(df_results["features"],
                                      df_results["total_features"])
  df_results["features_percent"] = (df_results["features"] /
                                    df_results["total_features"]) * 100.0
  ...
  df_results["env_name"] = df_results["env_name"].replace(
      "bookclub2", "bookclub")
  ...
  if include_tests:
    table_lines.append("".join([
        r"&  \multicolumn{2}{c|}{" + env_name_map[env_name] + "}"
        for env_name in df_out.env_name.unique()
    ]) + r"\\")
    table_lines.append(r"Method " + r"& \textbf{Features \%} & \textbf{LOC} " *
                       df_out.env_name.nunique() + r"\\")
    table_lines.append(r"" +
                       r"    & $\uparrow$ & " * df_out.env_name.nunique() +
                       r"\\")
  else:
    table_lines.append("".join([
        r"&  \multicolumn{2}{c|}{" + env_name_map[env_name] + "}"
      ...
      if method == "llmatic":
        if include_tests:
          if row is None or row.empty:
            ...
            line += (r"&\bf" +
                     f"{row.features_percent['mean'].iloc[0]:.{sf}g}" +
                     r"$\pm$" +
                     f"{row.features_percent[error_metric].iloc[0]:.{sf}g}" +
                     r"&\bf" + f"{row.locs['mean'].iloc[0]:,}" + r"$\pm$" +
                     f"{row.locs[error_metric].iloc[0]:.{sf}g}")
        else:
...
def generate_main_results_table_paper_format_tests_pass_human_eval(
    df_results,
    wandb=None,
    use_95_ci=True,
    seeds_to_use=None,
  ...
  df_results = remove_unneeded_columns(df_results)
  # df_results = df_results[df_results['locs'] != 0]
  env_name_to_total_features = {
      "url_shortener": 17,
      "whatsapp": 20,
      "twitter": 21,
      "recipe": 20,
  }
  df_results["total_features"] = df_results["env_name"].map(
      env_name_to_total_features)
  ...
  if rebuttal:  # Rebuttal
    ...
    df_results = df_results[df_results["env_name"] != "eventplanner"]
    df_results = df_results[df_results["env_name"] != "recipe"]
    unique_combinations_count = (df_results.groupby(
        ["env_name", "method"]).size().reset_index(name="count"))
    sampled_df = df_results.groupby([
        "env_name", "method"
    ]).apply(lambda x: x.sample(n=10, replace=False) if len(x) > 10 else x)
    sampled_df.reset_index(drop=True, inplace=True)
    sampled_df
    unique_combinations_count = (sampled_df.groupby(
        ["env_name", "method"]).size().reset_index(name="count"))
    print(unique_combinations_count)
    print("")
  ...
  if include_tests:
    table_lines.append("".join([
        r"&  \multicolumn{4}{c|}{" + env_name_map[env_name] + "}"
        for env_name in df_out.env_name.unique()
    ]) + r"\\")
    table_lines.append(
        r"Method " +
        r"& \textbf{Features \%} & \textbf{\# Errors} & \textbf{LOC} & \textbf{Passing Tests}"
        * df_out.env_name.nunique() + r"\\")
    table_lines.append(r"" +
    ...
    table_lines.append(r"Method " + r"& \textbf{Human Expert Features \%} " *
                       df_out.env_name.nunique() + r"\\")
    table_lines.append(r"" + r"    & $\uparrow$" * df_out.env_name.nunique() +
                       r"\\")
  table_lines.append(r"\midrule")
  for method in df_out.method.unique():
    if method == "llmatic":
        ...
        if include_tests:
          if row is None or row.empty:
            ...
            line += (r"&\bf" +
                     ...
                     r"$\pm$" +
                     f"{row.features_percent[error_metric].iloc[0]:.{sf}g}" +
                     r"&\bf" + f"{row.errors['mean'].iloc[0]:.{sf}g}" +
                     r"$\pm$" + f"{row.errors[error_metric].iloc[0]:.{sf}g}" +
                     r"&\bf" + f"{row.locs['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
                     f"{row.locs[error_metric].iloc[0]:.{sf}g}" + r"&\bf" +
                     f"{row.test_passed['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
                     f"{row.test_passed[error_metric].iloc[0]:.{sf}g}")
        else:
          if row is None or row.empty:
            line += r"& "
          else:
        ...
        if include_tests:
          if row is None or row.empty:
            ...
            line += (r"&" + f"{row.features_percent['mean'].iloc[0]:.{sf}g}" +
                     r"$\pm$" +
                     f"{row.features_percent[error_metric].iloc[0]:.{sf}g}" +
                     r"&" + f"{row.errors['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
                     f"{row.errors[error_metric].iloc[0]:.{sf}g}" + r"&" +
                     f"{row.locs['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
                     f"{row.locs[error_metric].iloc[0]:.{sf}g}" + r"&" +
                     f"{row.test_passed['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
                     f"{row.test_passed[error_metric].iloc[0]:.{sf}g}")
        else:
          if row is None or row.empty:
...
def generate_main_results_table_paper_format_tests_pass_human_eval_without_eb(
    df_results,
    wandb=None,
    use_95_ci=True,
    seeds_to_use=None,
...
def generate_main_results_table_paper_format_tests_pass_unit_tests_apriori(
    df_results,
    wandb=None,
    use_95_ci=True,
    seeds_to_use=None,
  ...
  df_results["features_percent"] = (df_results["features"] /
                                    df_results["total_features"]) * 100.0
  unit_test_total_input_dict = {
      "url_shortener": 9,
      "whatsapp": 19,
      "twitter": 13,
      "recipe": 20,
  }
  df_results["unit_test_total_input"] = df_results["env_name"].map(
      unit_test_total_input_dict)
  df_results["human_unit_test_percent"] = (
      df_results["unit_test_passed"] /
      df_results["unit_test_total_input"]) * 100.0
  df_results = df_results[df_results["env_name"] != "dropbox"]
  df_results = df_results[df_results["env_name"] != "recipe"]
  df_results = df_results[df_results["env_name"] != "whatsapp"]
  df_results = df_results[df_results["env_name"] != "twitter"]
  ...
  if True:  # Rebuttal
    ...
    sampled_df = df_results.groupby([
        "env_name", "method"
    ]).apply(lambda x: x.sample(n=5, replace=False) if len(x) > 5 else x)
    sampled_df.reset_index(drop=True, inplace=True)
    # sampled_df
    df_results = sampled_df
    unique_combinations_count = (sampled_df.groupby(
  ...
  if include_tests:
    table_lines.append("".join([
        r"&  \multicolumn{6}{c|}{" + env_name_map[env_name] + "}"
        for env_name in df_out.env_name.unique()
    ]) + r"\\")
    table_lines.append(
        r"Method " +
        r"& \textbf{Features \%} & \textbf{HT \%} & \textbf{\# Errors} & \textbf{LOC} & \textbf{Passing Tests} & \textbf{Cov \%}"
        * df_out.env_name.nunique() + r"\\")
    table_lines.append(
        r"" +
        r"    & $\uparrow$ & $\uparrow$ & $\downarrow$ & & $\uparrow$ & $\uparrow$ "
        * df_out.env_name.nunique() + r"\\")
  else:
    table_lines.append("".join([
        r"&  \multicolumn{3}{c|}{" + env_name_map[env_name] + "}"
        for env_name in df_out.env_name.unique()
    ]) + r"\\")
    table_lines.append(
        r"Method " +
        r"& \textbf{Features \%} & \textbf{\# Errors} & \textbf{LOC} " *
        df_out.env_name.nunique() + r"\\")
  table_lines.append(r"\midrule")
  for method in df_out.method.unique():
    if method == "llmatic":
      ...
      if method == "llmatic":
        if include_tests:
          if row is None or len(row) == 0:
            ...
            line += (
                r"&\bf" + f"{row.features_percent['mean'].iloc[0]:.{sf}g}" +
                r"$\pm$" +
                f"{row.features_percent[error_metric].iloc[0]:.{sf}g}" +
                r"&\bf" +
                f"{row.human_unit_test_percent['mean'].iloc[0]:.{sf}g}" +
                r"$\pm$" +
                f"{row.human_unit_test_percent[error_metric].iloc[0]:.{sf}g}" +
                r"&\bf" + f"{row.errors['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
                f"{row.errors[error_metric].iloc[0]:.{sf}g}" + r"&\bf" +
                f"{row.locs['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
                f"{row.locs[error_metric].iloc[0]:.{sf}g}" + r"&\bf" +
                f"{row.test_passed['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
                f"{row.test_passed[error_metric].iloc[0]:.{sf}g}" + r"&\bf" +
                f"{row.coverage_percent['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
                f"{row.coverage_percent[error_metric].iloc[0]:.{sf}g}")
        else:
          line += (r"&\bf" + f"{row.features_percent['mean'].iloc[0]:.{sf}g}" +
                   r"$\pm$" +
                   f"{row.features_percent[error_metric].iloc[0]:.{sf}g}" +
        ...
        if include_tests:
          if row is None or len(row) == 0:
            ...
            line += (
                r"&" + f"{row.features_percent['mean'].iloc[0]:.{sf}g}" +
                r"$\pm$" +
                f"{row.features_percent[error_metric].iloc[0]:.{sf}g}" + r"&" +
                f"{row.human_unit_test_percent['mean'].iloc[0]:.{sf}g}" +
                r"$\pm$" +
                f"{row.human_unit_test_percent[error_metric].iloc[0]:.{sf}g}" +
                r"&" + f"{row.errors['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
                ...
                f"{row.locs[error_metric].iloc[0]:.{sf}g}" + r"&" +
                f"{row.test_passed['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
                f"{row.test_passed[error_metric].iloc[0]:.{sf}g}" + r"&" +
                f"{row.coverage_percent['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
                f"{row.coverage_percent[error_metric].iloc[0]:.{sf}g}")
        else:
          line += (r"&" + f"{row.features_percent['mean'].iloc[0]:.{sf}g}" +
...
def generate_main_results_table_paper_format_tests_pass_unit_tests_apriori_without_errors_or_passing_tests(
    df_results,
    wandb=None,
    use_95_ci=True,
    seeds_to_use=None,
  ...
  if include_tests:
    ...
    table_lines.append(
        r"Method " +
        r"& \textbf{Features \%} & \textbf{HT \%} & \textbf{LOC} & \textbf{Cov \%}"
        * df_out.env_name.nunique() + r"\\")
    table_lines.append(r"" + r"    & $\uparrow$ & $\uparrow$ & & $\uparrow$ " *
                       df_out.env_name.nunique() + r"\\")
  else:
    table_lines.append("".join([
        r"&  \multicolumn{3}{c|}{" + env_name_map[env_name] + "}"
        ...
        if include_tests:
          if row is None or len(row) == 0:
            ...
            line += (
                ...
                r"$\pm$" +
                f"{row.human_unit_test_percent[error_metric].iloc[0]:.{sf}g}" +
                r"&" + f"{row.locs['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
                f"{row.locs[error_metric].iloc[0]:.{sf}g}" + r"&" +
                f"{row.coverage_percent['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
                f"{row.coverage_percent[error_metric].iloc[0]:.{sf}g}")
        else:
...
def generate_main_results_table_paper_format(
    df_results,
    wandb=None,
    use_95_ci=True,
    seeds_to_use=None,
  ...
  if include_tests:
    ...
    table_lines.append(
        r"Method " +
        r"& \textbf{Features \%} & \textbf{\# Errors} & \textbf{LOC} & \textbf{Test \%}"
        * df_out.env_name.nunique() + r"\\")
  else:
    table_lines.append("".join([
        r"&  \multicolumn{3}{c|}{" + env_name_map[env_name] + "}"
      ...
      if method == "llmatic":
        if include_tests:
          line += (r"&\bf" + f"{row.features_percent['mean'].iloc[0]:.{sf}g}" +
                   ...
                   f"{row.locs['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
                   f"{row.locs[error_metric].iloc[0]:.{sf}g}" + r"&\bf" +
                   f"{row.test_percent['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
                   f"{row.test_percent[error_metric].iloc[0]:.{sf}g}")
        else:
          line += (r"&\bf" + f"{row.features_percent['mean'].iloc[0]:.{sf}g}" +
                   r"$\pm$" +
                   f"{row.features_percent[error_metric].iloc[0]:.{sf}g}" +
...
def generate_main_results_table(
    ...
    wandb=None,
    use_95_ci=True,
    return_all_next_step_head_n_tables=True,
):
  # Process seeds here
  df_results = remove_unneeded_columns(df_results)
  if use_95_ci:
  ...
  table_lines.append(r"\toprule")
  table_lines.append("".join([
      r"&  " + env_name_map[env_name] for env_name in df_out.env_name.unique()
  ]) + r"\\")
  table_lines.append(r"Policy " +
                     r"$\mathcal{R}$ " * df_out.env_name.nunique() + r"\\")
  table_lines.append(r"\midrule")
  for method in df_out.method.unique():
    ...
    for env_name in df_out.env_name.unique():
      ...
      if method == "relentless":
        line += (r"&\bf" + f"{row.reward['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
                 f"{row.reward[error_metric].iloc[0]:.{sf}g}")
      else:
        line += (r"&" + f"{row.reward['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
                 f"{row.reward[error_metric].iloc[0]:.{sf}g}")
    line += r"\\"
    table_lines.append(line)
  table_lines.append(r"\bottomrule")
...
def generate_n_step_graph(df_results, wandb=None, use_95_ci=True):
  ...
  if use_95_ci:
    df_out = (df_results.groupby(["env_name", "method",
                                  "gamma"]).agg([np.mean, ci]).reset_index())
    error_metric = "ci"
  else:
    df_out = (df_results.groupby(["env_name", "method",
                                  "gamma"]).agg([np.mean,
...
def generate_n_step_graph(df_results, wandb=None, use_95_ci=True):
  df_results = df_results.drop(
      columns=["global_equation_string", "fine_tuned", "method"])
  # Process seeds here
  df_results = remove_unneeded_columns(df_results)
  if use_95_ci:
    df_out = (df_results.groupby(["env_name", "method",
                                  "domain_conf"]).agg([np.mean,
                                                       ci]).reset_index())
    error_metric = "ci"
  else:
    df_out = (df_results.groupby(["env_name", "method",
  ...
  for env_name in df_out.env_name.unique():
    for decoder_test_step in list(
        set(l[0]
            for l in list(df_out.columns)
            if "decoder_test_rmse_" in l[0])):
      for method in df_out.method.unique():
        df = df_out[df_out.env_name == env_name]
  ...
  steps = list(
      set(l[0] for l in list(df_out.columns) if "decoder_test_rmse_" in l[0]))
  ...
  for method in df_out.method.unique():
    ...
    data_dict[method]["std"].append([(y_meani - y_stdi, y_meani + y_stdi)
                                     for y_meani, y_stdi in zip(y_mean, y_std)])
...
def generate_overlap_graph(df_results, wandb=None, use_95_ci=True):
  ...
  for env_name in df_out.env_name.unique():
    ...
    for method in df_out.method.unique():
      ...
      # plt.ylim(bottom=0)
      data_dict[method]["res"].append([(xi, yi) for xi, yi in zip(x, y_mean)])
      data_dict[method]["std"].append([
          (y_meani - y_stdi, y_meani + y_stdi)
          for y_meani, y_stdi in zip(y_mean, y_std)
      ])
...
def plot_less_samples(df, use_95_ci=True):
  ...
  for env_name in df.env_name.unique():
    ...
    for method in df_t.method.unique():
      ...
      df_t[df_t.method == method][y_metric][error_metric]
...
def plot_evolutionary_plots_inner_single(df, new_elements, use_95_ci=True):
  ...
  x = x[:15]
  y_mean = y_mean[:15]
  y_other = np.array(new_elements)[:10]
  # y_std = df_t[y_metric][error_metric]
  # plt.plot(x,y_mean,'--o', label='Top-1 D3')
  plt.plot(np.arange(1,
                     len(y_other) + 1),
           y_other,
           "--o",
           label="Top-1 DI3 Model")
  # plt.fill_between(x,y_mean - y_std,y_mean + y_std,alpha=0.25)
  ...
  plt.tight_layout()
  plt.savefig(f"./plots/D3_indvidual_{seed}_generation_plot.png")
  plt.savefig(f"./plots/D3_indvidual_{seed}_generation_plot.pdf")
  print(f"./plots/D3_indvidual_{seed}_generation_plot.png")
  plt.clf()
  print("")
...
def plot_evolutionary_plots_inner(df, use_95_ci=True):
  ...
  # plt.tight_layout()
  plt.savefig("./plots/D3_generation_plot.png")
  plt.savefig("./plots/D3_generation_plot.pdf")
  print("./plots/D3_generation_plot.png")
  plt.clf()
  print("")
...
def plot_intervention_COVID():
  ...
  d3_data = np.load("COVID-intervention.npz")
  sindy_data = np.load("COVID-intervention-sindy.npz")
  dynode_data = np.load("COVID-intervention-dynode.npz")

  noint_states = d3_data["noint_states"]
  int_states = d3_data["int_states"]
  d3_predicted_int_states = d3_data["d3_predicted_int_states"]
  d3_data["d3_predicted_full_states"]
  ...
  sindy_offset = (N * int_states[traj_idx, intervention_t:, dim] -
                  N * predicted_sindy_states[traj_idx, intervention_t:, dim])[0]
  dynode_offset = (
      N * int_states[traj_idx, intervention_t:, dim] -
      N * predicted_dynode_states[traj_idx, intervention_t:, dim])[0]

  plt.plot(
      t[:intervention_t + 1],
      N * noint_states[traj_idx, :intervention_t + 1, dim],
      color="grey",
  )
  plt.plot(
      t[intervention_t:],
  ...
  plt.plot(
      t[intervention_t:],
      N * d3_predicted_int_states[traj_idx, intervention_t:, dim],
      "--o",
      label="NDST (with intervention)",
  )
  # plt.plot(t[intervention_t:], N*d3_predicted_full_states[traj_idx,:,dim],'--o',label="NDST")
  plt.plot(
      t[intervention_t:],
      N * predicted_sindy_states[traj_idx, intervention_t:, dim] + sindy_offset,
      "--o",
      label="SINDy",
  )
  plt.plot(
      t[intervention_t:],
      N * predicted_dynode_states[traj_idx, intervention_t:, dim] +
      dynode_offset,
      "--o",
      label="DyNODE",
  )
  ...
  # plt.tight_layout()
  plt.savefig("./plots/COVID-intervention.png")
  plt.savefig("./plots/COVID-intervention.pdf")
  print("./plots/COVID-intervention.png")
  plt.clf()
  print("")
...
def plot_threshold_plots(df, use_95_ci=True):
  ...
  thresholds_used = {
      "oderl-cancer": 6.760299902695876,
  ...
  for env_name in df.env_name.unique():
    ...
    x = df_t[df_t.method == "active_observing_control"][x_metric]  # ['mean']
    y_mean = df_t[df_t.method == "active_observing_control"][y_metric]["mean"]
    y_std = df_t[df_t.method ==
                 "active_observing_control"][y_metric][error_metric]
    plt.plot(x, y_mean, "--o", label=method_map["active_observing_control"])
    plt.fill_between(x, y_mean - y_std, y_mean + y_std, alpha=0.25)
    cp_y_mean = df_t[df_t.method ==
                     "continuous_planning"][y_metric]["mean"].iloc[0]
    cp_y_mean = np.ones_like(y_mean) * cp_y_mean
    cp_y_std = df_t[df_t.method ==
                    "continuous_planning"][y_metric][error_metric].iloc[0]
    cp_y_std = np.ones_like(y_mean) * cp_y_std
    plt.plot(x, cp_y_mean, "--o", label=method_map["continuous_planning"])
    plt.fill_between(x, cp_y_mean - cp_y_std, cp_y_mean + cp_y_std, alpha=0.25)
    plt.ylabel(r"$\mathcal{U}$")
  ...
  for line in tqdm(lines):
    if "[Exp evaluation complete] {" in line:
      ...
      if result_dict["errored"]:
        print(f"skipping: {result_dict['env_name']} seed {result_dict['seed']}")
        continue
...
def load_df_file_evolutionary_plots(path, remove_extra_columns=True):
  ...
  for line in tqdm(lines):
    ...
    # if '[D3-feature-select | Cancer-random-features |' in line:
    #     pattern = r'\[(?P<method>[^\|]+) \| (?P<env_name>[^\|]+) \| (?P<seed>\d+)\]\[Generation (?P<generation>\d+)\].*Fitnesses: \[(?P<fitnesses>[^\]]+)\]'
    ...
    if "[D3 | warfarin |" in line:
      ...
      if match:
        ...
        # Convert the 'fitnesses' string to a list of floats
        extracted["fitnesses"] = [
            float(f) for f in extracted["fitnesses"].split(",")
        ]
