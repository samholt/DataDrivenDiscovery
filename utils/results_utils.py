        ...
        if "[Exp evaluation complete] {" in line:
            ...
            if load_tensorboard_data:
                if "run_name" in result_dict:
                    ...
                    log_path = file_path_from_parent_directory(
                        f"./runs/{run_name}"
                    )
                    tensorboard_data = load_metrics_from_tensorboard_file(
                        log_path
                    )
...
def normalize_means(df):
    df_means = (
        df.groupby(["env_name", "policy", "network_specific"])
        .agg(np.mean)
        .reset_index()
    )
    for env_name in df_means.env_name.unique():
        ...
        best_row = df_means_env[
            df_means_env.method == "continuous_planning"
        ].iloc[0]
...
def compute_norm_metrics(df):
    ...
    for env_name in df.env_name.unique():
        ...
        df.loc[df.env_name == env_name, "test_rmse"] = (
            df[df.env_name == env_name].test_rmse / norm
        )
    return df


def generate_main_results(
    df_results, wandb=None, use_95_ci=True, seeds_to_use=None
):
    # Process seeds here
    ...
    if use_95_ci:
        df_out = (
            df_results.groupby(["env_name", "method"])
            .agg([np.mean, ci])
            .reset_index()
        )
        error_metric = "ci"
    ...
    df_out = df_out.sort_values(
        ...
        if col.name == "env_name"
        else col.map(method_name_ordering),
    )
    table_lines = []
    ...
    table_lines.append(
        r"Method " + r"& \textbf{Test MSE} " * df_out.env_name.nunique() + r"\\"
    )
    table_lines.append(
        r"" + r"    & $\downarrow$ " * df_out.env_name.nunique() + r"\\"
    )
    ...
    for method in df_out.method.unique():
        ...
        for env_name in df_out.env_name.unique():
            row = df_out[
                (df_out.method == method) & (df_out.env_name == env_name)
            ]
            if row.empty:
                line += r"&" + "WIP" + r"$\pm$" + "WIP"
            elif method == "D3":
                line += (
                    r"&\bf"
                    + f"{row.test_mse['mean'].iloc[0]:.{sf}g}"
                    + r"$\pm$"
...
def generate_main_results_ood_table(
    df_results, wandb=None, use_95_ci=True, seeds_to_use=None
):
    # Process seeds here
    df_results = remove_unneeded_columns(df_results)
...
def generate_main_results_less_samples(
    df_results, wandb=None, use_95_ci=True, seeds_to_use=None
):
    # Process seeds here
    df_results = remove_unneeded_columns(df_results)
    if use_95_ci:
        df_out = (
            df_results.groupby(["env_name", "method", "trajectories"])
            .agg([np.mean, ci])
            .reset_index()
        )
        error_metric = "ci"
    ...
    df_out = df_out.sort_values(
        ...
        key=lambda col: col.map(env_name_ordering)
        if col.name == "env_name"
        else col.map(plot_method_name_ordering),
    )
...
def generate_main_results_table_paper_format_tests_pass_feature_implement(
    ...
    df_results["features"] = np.minimum(
        df_results["features"], df_results["total_features"]
    )
    df_results["features_percent"] = (
        df_results["features"] / df_results["total_features"]
    ) * 100.0
    ...
    if include_tests:
        ...
        table_lines.append(
            r"Method "
            + r"& \textbf{Features \%} & \textbf{LOC} "
            * df_out.env_name.nunique()
            + r"\\"
        )
        table_lines.append(
            r"" + r"    & $\uparrow$ & " * df_out.env_name.nunique() + r"\\"
        )
    else:
        table_lines.append(
            "".join(
...
def generate_main_results_table_paper_format_tests_pass_human_eval(
    ...
    if rebuttal:  # Rebuttal
        ...
        sampled_df = df_results.groupby(["env_name", "method"]).apply(
            lambda x: x.sample(n=10, replace=False) if len(x) > 10 else x
        )
        sampled_df.reset_index(drop=True, inplace=True)
        sampled_df
        unique_combinations_count = (
            sampled_df.groupby(["env_name", "method"])
            .size()
            .reset_index(name="count")
        )
        print(unique_combinations_count)
    ...
    if include_tests:
        line = (
            r"\begin{tabular}{@{}l" + "|cccc" * df_out.env_name.nunique() + "}"
        )
    else:
        line = r"\begin{tabular}{@{}l" + "|c" * df_out.env_name.nunique() + "}"
    table_lines.append(line)
                ...
                if include_tests:
                    if row is None or row.empty:
                        ...
                        line += (
                            ...
                            + f"{row.features_percent[error_metric].iloc[0]:.{sf}g}"
                            + r"&\bf"
                            + f"{row.errors['mean'].iloc[0]:.{sf}g}"
                            + r"$\pm$"
                            + f"{row.errors[error_metric].iloc[0]:.{sf}g}"
                            + r"&\bf"
                            + f"{row.locs['mean'].iloc[0]:.{sf}g}"
                            + r"$\pm$"
                            + f"{row.locs[error_metric].iloc[0]:.{sf}g}"
                            + r"&\bf"
                            + f"{row.test_passed['mean'].iloc[0]:.{sf}g}"
                            + r"$\pm$"
...
def generate_main_results_table_paper_format_tests_pass_unit_tests_apriori(
    ...
    df_results["human_unit_test_percent"] = (
        df_results["unit_test_passed"] / df_results["unit_test_total_input"]
    ) * 100.0
    df_results = df_results[df_results["env_name"] != "dropbox"]
    df_results = df_results[df_results["env_name"] != "recipe"]
    df_results = df_results[df_results["env_name"] != "whatsapp"]
    ...
    if True:  # Rebuttal
        ...
        # unique_combinations_count = df_results.groupby(['env_name', 'method']).size().reset_index(name='count')
        sampled_df = df_results.groupby(["env_name", "method"]).apply(
            lambda x: x.sample(n=5, replace=False) if len(x) > 5 else x
        )
        sampled_df.reset_index(drop=True, inplace=True)
        # sampled_df
        df_results = sampled_df
    ...
    if include_tests:
        line = (
            r"\begin{tabular}{@{}l"
            + "|cccccc" * df_out.env_name.nunique()
            + "}"
        )
    else:
        line = (
            r"\begin{tabular}{@{}l" + "|ccc" * df_out.env_name.nunique() + "}"
        )
    table_lines.append(line)
    table_lines.append(r"\toprule")
    if include_tests:
                ...
                if include_tests:
                    if row is None or len(row) == 0:
                        ...
                        line += (
                            ...
                            + f"{row.test_passed[error_metric].iloc[0]:.{sf}g}"
                            + r"&\bf"
                            + f"{row.coverage_percent['mean'].iloc[0]:.{sf}g}"
                            + r"$\pm$"
            ...
            if method == "llmatic":
                if include_tests:
                    line += (
                        ...
                        + f"{row.locs[error_metric].iloc[0]:.{sf}g}"
                        + r"&\bf"
                        + f"{row.test_percent['mean'].iloc[0]:.{sf}g}"
                        + r"$\pm$"
...
def generate_main_results_table(
    ...
    table_lines.append(
        r"Policy " + r"$\mathcal{R}$ " * df_out.env_name.nunique() + r"\\"
    )
    table_lines.append(r"\midrule")
    for method in df_out.method.unique():
...
def generate_n_step_graph(df_results, wandb=None, use_95_ci=True):
    ...
    if use_95_ci:
        df_out = (
            df_results.groupby(["env_name", "method", "gamma"])
            .agg([np.mean, ci])
            .reset_index()
        )
        error_metric = "ci"
...
def generate_n_step_graph(df_results, wandb=None, use_95_ci=True):
    ...
    if use_95_ci:
        df_out = (
            df_results.groupby(["env_name", "method", "domain_conf"])
            .agg([np.mean, ci])
            .reset_index()
        )
        error_metric = "ci"
...
def generate_overlap_graph(df_results, wandb=None, use_95_ci=True):
    ...
    for env_name in df_out.env_name.unique():
        ...
        for method in df_out.method.unique():
            ...
            for decoder_test_step in steps:
                ...
                y_mean.append(
                    df[decoder_test_step]["mean"].to_numpy()[d_idx][0]
                )
                y_std.append(
                    df[decoder_test_step][error_metric].to_numpy()[d_idx][0]
                )
            x = np.array(x)
            y_mean = np.array(y_mean)
            y_std = np.array(y_std)
            ...
            plt.fill_between(x, y_mean - y_std, y_mean + y_std, alpha=0.25)
            # plt.ylim(bottom=0)
            data_dict[method]["res"].append(
                [(xi, yi) for xi, yi in zip(x, y_mean)]
            )
            data_dict[method]["std"].append(
                [
                    (y_meani - y_stdi, y_meani + y_stdi)
    ...
    for env_name in df_out.env_name.unique():
        for decoder_test_step in steps:
            ...
            plt.savefig(
                f"./results/domain_conf_{env_name}_{decoder_test_step}.png"
            )
            plt.savefig(
                f"./results/domain_conf_{env_name}_{decoder_test_step}.pdf"
            )
...
def plot_evolutionary_plots_inner_single(df, new_elements, use_95_ci=True):
    ...
    plt.plot(
        np.arange(1, len(y_other) + 1), y_other, "--o", label="Top-1 DI3 Model"
    )
    # plt.fill_between(x,y_mean - y_std,y_mean + y_std,alpha=0.25)
...
def plot_intervention_COVID():
    ...
    sindy_offset = (
        ...
        - N * predicted_sindy_states[traj_idx, intervention_t:, dim]
    )[0]
    dynode_offset = (
        N * int_states[traj_idx, intervention_t:, dim]
...
def plot_threshold_plots(df, use_95_ci=True):
    ...
    for env_name in df.env_name.unique():
        ...
        x = df_t[df_t.method == "active_observing_control"][
            x_metric
        ]  # ['mean']
        y_mean = df_t[df_t.method == "active_observing_control"][y_metric][
            "mean"
        ]
        y_std = df_t[df_t.method == "active_observing_control"][y_metric][
            error_metric
        ]
        plt.plot(x, y_mean, "--o", label=method_map["active_observing_control"])
        plt.fill_between(x, y_mean - y_std, y_mean + y_std, alpha=0.25)
        cp_y_mean = df_t[df_t.method == "continuous_planning"][y_metric][
            "mean"
        ].iloc[0]
        cp_y_mean = np.ones_like(y_mean) * cp_y_mean
        cp_y_std = df_t[df_t.method == "continuous_planning"][y_metric][
            error_metric
        ].iloc[0]
        cp_y_std = np.ones_like(y_mean) * cp_y_std
        plt.plot(x, cp_y_mean, "--o", label=method_map["continuous_planning"])
        plt.fill_between(
            x, cp_y_mean - cp_y_std, cp_y_mean + cp_y_std, alpha=0.25
        )
        plt.ylabel(r"$\mathcal{U}$")
        plt.axvline(x=threshold_we_used, color="r")
    ...
    for line in tqdm(lines):
        if "[Exp evaluation complete] {" in line:
            ...
            if result_dict["errored"]:
                print(
                    f"skipping: {result_dict['env_name']} seed {result_dict['seed']}"
                )
                continue
