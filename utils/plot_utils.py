...
def df_from_log(path, remove_extra_columns=True):
    ...
    dfm[numeric_columns] = dfm[numeric_columns].apply(
...
def normalize_means(df):
    df_means = (
        df.groupby(["env_name", "sampling_policy", "model_name"])
        .agg(np.mean)
        .reset_index()
    )
    for env_name in df_means.env_name.unique():
        ...
        random_row = df_means_env[
            df_means_env.sampling_policy == "random"
        ].iloc[0]
        best_row = df_means_env[
            df_means_env.sampling_policy == "continuous_planning"
        ].iloc[0]
...
def generate_main_results_table_cp_with_unique_obs(df_results, wandb=None):
    ...
    df_out = (
        df_results.groupby(["env_name", "sampling_policy", "model_name"])
        .agg([np.mean, np.std])
        .reset_index()
    )
    ...
    df_out = df_out.sort_values(
        by=["env_name"], key=lambda x: x.map(env_name_ordering)
    )
    table_lines = []
    ...
    for sampling_policy in df_out.sampling_policy.unique():
        ...
        for env_name in df_out.env_name.unique():
            row = df_out[
                (df_out.sampling_policy == sampling_policy)
        ...
        for env_name in df_out.env_name.unique():
            ...
            if sampling_policy == "active_observing_control":
                line += (
                    (
                        r"&\bf"
                        if row.total_reward["mean"].iloc[0] > 100.0
                        else r"&"
                    )
                    + f"{row.total_reward['mean'].iloc[0]:.{sf}g}"
                    + r"$\pm$"
                    ...
                    + (
                        r"&\bf"
                        if row.state_reward["mean"].iloc[0] > 100.0
                        else r"&"
                    )
                    + f"{row.state_reward['mean'].iloc[0]:.{sf}g}"
                    + r"$\pm$"
...
def plot_error_plots(results):
    ...
    for env_name in df.env_name.unique():
        ...
        y_mean = df_t[df_t.sampling_policy == "active_observing_control"][
            y_metric
        ]["mean"]
        y_std = df_t[df_t.sampling_policy == "active_observing_control"][
            y_metric
        ][error_metric]
        plt.plot(
            x,
        ...
        cp_y_mean = df_t[df_t.sampling_policy == "continuous_planning"][
            y_metric
        ]["mean"].iloc[0]
        cp_y_mean = np.ones_like(y_mean) * cp_y_mean
        cp_y_std = df_t[df_t.sampling_policy == "continuous_planning"][
            y_metric
        ][error_metric].iloc[0]
...
def state_cancer_hist_v2(df):
    ...
    obs_count, state_bins_out = np.histogram(
        df_cp_row.s[df_cp_row.observed_times, 0], bins=state_bins
    )
    s = df_cp_row.s[:, 0]
...
def state_cancer_hist(df):
    ...
    count, values = np.histogram(
        states_observed, bins=np.linspace(0, 1, 5) * 1140
    )
...
def plot_pendulum_state(df):
    ...
    for i, row in df.iterrows():
        ...
        if i <= 1:
            plt.plot(
                st[:, 0], st[:, 1], color="k", linewidth=2.0
            )  # , marker='o', alpha=.5,)
    ...
    h_obs, xedges_obs, yedged_obs = np.histogram2d(
        x=x_obs, y=y_obs, bins=(x_bins, y_bins)
    )
    # np.histogram2d(x=all_states[:,0], y=all_states[:,1], bins=hist_bins)
    h_all, xedges_all, yedged_all = np.histogram2d(
        x=x_all, y=y_all, bins=(x_bins, y_bins)
    )
    h_obs_per_time = np.nan_to_num((h_obs / h_all))
    plt.imshow(
        h_obs_per_time,
...
def plot_match_cp_obs_plots(df):
    ...
    for env_name in df_standard.env_name.unique():
        ...
        x = df_standard_t[
            df_standard_t.sampling_policy == "active_observing_control"
        ][x_metric]["mean"]
        y_mean = df_standard_t[
            df_standard_t.sampling_policy == "active_observing_control"
        ][y_metric]["mean"]
        y_std = df_standard_t[
            df_standard_t.sampling_policy == "active_observing_control"
        ...
        x = cp_dif_obs_t[cp_dif_obs_t.sampling_policy == "continuous_planning"][
            x_metric
        ]["mean"]
        print(x)
        y_mean = cp_dif_obs_t[
            cp_dif_obs_t.sampling_policy == "continuous_planning"
        ][y_metric]["mean"]
        y_std = cp_dif_obs_t[
            cp_dif_obs_t.sampling_policy == "continuous_planning"
        ][y_metric]["std"]
        plt.plot(
            x, y_mean, "--o", label=sampling_policy_map["continuous_planning"]
        )
        plt.fill_between(x, y_mean - y_std, y_mean + y_std, alpha=0.25)
...
def cost_std_comparison(df):
    ...
    HEIGHT_SCALE = 0.30
    1 / HEIGHT_SCALE  # -(0.05 + LEGEND_Y_CORD)
    sn = configure_plotting_sn_params(
        sn, SCALE, HEIGHT_SCALE, use_autolayout=False
    )
    plt.gcf().subplots_adjust(bottom=0.40, left=0.2, top=0.95)

    sampling_policy_map = {
        "discrete_monitoring": r"Discrete Monitoring \quad\quad\quad $\mathcal{R}=85.7\pm0.526$",
