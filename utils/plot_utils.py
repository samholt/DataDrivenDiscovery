...
def get_normalized_policy_values_delay_zero():
  random_policy = {
      "oderl-acrobot": -2948.636826752257,
      "oderl-cartpole": -14246.301963850627,
      "oderl-pendulum": -616.7659306662474,
  }
  best_policy = {
      "oderl-acrobot": -571.1055129432718,
      "oderl-cartpole": -139.68956484338668,
      "oderl-pendulum": -121.04611233502484,
  }
...
def df_from_log(path, remove_extra_columns=True):
  ...
  dfm[numeric_columns] = dfm[numeric_columns].apply(pd.to_numeric,
                                                    errors="coerce")
...
def normalize_means(df):
  df_means = (df.groupby(["env_name", "sampling_policy",
                          "model_name"]).agg(np.mean).reset_index())
  for env_name in df_means.env_name.unique():
    ...
    best_row = df_means_env[df_means_env.sampling_policy ==
...
def normalize_means_cp_matching_obs(df_in):
  ...
  for env_name in df_means.env_name.unique():
    ...
    df_in.loc[df_in.env_name == env_name, "total_reward"] = (
        (df_in[df_in.env_name == env_name].total_reward -
         random_row.total_reward) /
        (best_row.total_reward - random_row.total_reward)) * 100.0
    df_in.loc[df_in.env_name == env_name, "state_reward"] = (
        (df_in[df_in.env_name == env_name].state_reward -
         random_row.state_reward) /
        (best_row.state_reward - random_row.state_reward)) * 100.0
  return df_in
...
def generate_main_results_table_cp_with_unique_obs(df_results, wandb=None):
  ...
  df_out = (df_results.groupby(["env_name", "sampling_policy",
                                "model_name"]).agg([np.mean,
                                                    np.std]).reset_index())
  ...
  df_out = df_out.sort_values(by=["env_name"],
                              key=lambda x: x.map(env_name_ordering))
  table_lines = []
  ...
  table_lines.append(r"Policy " +
                     r"& $\mathcal{U}$ & $\mathcal{R}$ & $\mathcal{O}$" *
  ...
  for sampling_policy in df_out.sampling_policy.unique():
    ...
    for env_name in df_out.env_name.unique():
      row = df_out[(df_out.sampling_policy == sampling_policy) &
                   (df_out.env_name == env_name)]
      line += (r"&" + f"{row.total_reward['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
               f"{row.total_reward['std'].iloc[0]:.{sf}g}" + r"&" +
               f"{row.state_reward['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
               f"{row.state_reward['std'].iloc[0]:.{sf}g}" + r"&" +
               f"{row.observations_taken['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
    ...
    for env_name in df_out.env_name.unique():
      ...
      if sampling_policy == "active_observing_control":
        line += (
            ...
            f"{row.total_reward['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
            f"{row.total_reward[error_metric].iloc[0]:.{sf}g}" +
            (r"&\bf" if row.state_reward["mean"].iloc[0] > 100.0 else r"&") +
            f"{row.state_reward['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
            f"{row.state_reward[error_metric].iloc[0]:.{sf}g}" + r"&" +
            f"{row.observations_taken['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
            f"{row.observations_taken[error_metric].iloc[0]:.{sf}g}")
      else:
        line += (r"&" + f"{row.total_reward['mean'].iloc[0]:.{sf}g}" +
                 r"$\pm$" + f"{row.total_reward[error_metric].iloc[0]:.{sf}g}" +
                 r"&" + f"{row.state_reward['mean'].iloc[0]:.{sf}g}" +
                 r"$\pm$" + f"{row.state_reward[error_metric].iloc[0]:.{sf}g}" +
                 r"&" + f"{row.observations_taken['mean'].iloc[0]:.{sf}g}" +
                 r"$\pm$" +
                 f"{row.observations_taken[error_metric].iloc[0]:.{sf}g}")
    line += r"\\"
    table_lines.append(line)
...
def plot_error_plots(results):
  ...
  print("")
  autogpt.change_count.max()
  plt.plot(
      (llmatic.change_count / llmatic.change_count.max()) * 100.0,
      llmatic.errors,
      "--o",
      label="Code-L2MAC",
      alpha=0.7,
  ...
  plt.plot(
      (autogpt.change_count / autogpt.change_count.max()) * 100.0,
      autogpt.errors,
      "--o",
      label="AutoGPT",
      alpha=0.7,
  ...
  plt.savefig("./results/features.pdf")
  print("./results/features.png")
  plt.clf()
  ...
  # plt.tight_layout()
  plt.savefig("./results/errors_legend.png")
  plt.savefig("./results/errors_legend.pdf")
  print("./results/errors_legend.png")
  plt.clf()
  ...
  for ax, method in zip(axes, methods):
    ...
    ax.bar(
        x_values,
        subset["tests_total"] - subset["tests_passed"],
        width=bar_width,
        label="Tests Failed",
        color=beautiful_red,
        bottom=subset["tests_passed"],
        alpha=0.7,
        align="edge",
    )
  ...
  plt.savefig("./results/chatgpt.png")
  plt.savefig("./results/chatgpt.pdf")
  print("./results/chatgpt.png")
  plt.clf()
  ...
  plt.savefig("./results/chatgpt_legend.png")
  plt.savefig("./results/chatgpt_legend.pdf")
  print("./results/chatgpt_legend.png")
  plt.clf()
  ...
  for env_name in df.env_name.unique():
    ...
    x = df_t[df_t.sampling_policy == "active_observing_control"][
        x_metric]  # ['mean']
    y_mean = df_t[df_t.sampling_policy ==
                  "active_observing_control"][y_metric]["mean"]
    y_std = df_t[df_t.sampling_policy ==
                 "active_observing_control"][y_metric][error_metric]
    plt.plot(
        x,
        y_mean,
        "--o",
        label=sampling_policy_map["active_observing_control"],
    )
    plt.fill_between(x, y_mean - y_std, y_mean + y_std, alpha=0.25)
    cp_y_mean = df_t[df_t.sampling_policy ==
                     "continuous_planning"][y_metric]["mean"].iloc[0]
    cp_y_mean = np.ones_like(y_mean) * cp_y_mean
    cp_y_std = df_t[df_t.sampling_policy ==
    ...
    plt.plot(
        ...
        cp_y_mean,
        "--o",
        label=sampling_policy_map["continuous_planning"],
    )
...
def plot_write_heatmaps(data):
  ...
  plt.legend(
      handles=[black_patch, white_patch, red_patch, blue_patch],
      fancybox=True,
      shadow=True,
  )

  plt.savefig("./results/legend_memory.png")
  plt.savefig("./results/legend_memory.pdf")
  print("./results/legend_memory.png")
  plt.clf()
  print("")
  ...
  plt.plot(
      (llmatic.change_count / llmatic.change_count.max()) * 100.0,
      np.ones(llmatic.change_count.shape[0]) * 100.0,
      "--o",
      label="Code-L2MAC",
      alpha=0.7,
  )
...
def state_cancer_hist_v2(df):
  ...
  observing_var_threshold = observing_var_thresholds[df_cp_row.dt_plan][
      df_cp_row.env_name]["continuous"]

  obs_count, state_bins_out = np.histogram(df_cp_row.s[df_cp_row.observed_times,
                                                       0],
                                           bins=state_bins)
  s = df_cp_row.s[:, 0]
  ...
  plt.savefig(
      f"./plots/{df_cp_row.env_name}_hist_with_state_{df_cp_row.sampling_policy}.png"
  )
  plt.savefig(
      f"./plots/{df_cp_row.env_name}_hist_with_state_{df_cp_row.sampling_policy}.pdf"
  )
  plt.clf()
  ...
  plt.ylabel(r"$\frac{\mathcal{O}}{t}$ $(\frac{\mathrm{obs}}{\mathrm{day}})$")
  plt.savefig(
      f"./plots/{row.env_name}_hist_with_state_{row.sampling_policy}.png")
  plt.savefig(
      f"./plots/{row.env_name}_hist_with_state_{row.sampling_policy}.pdf")
  plt.clf()
  file_path = (
      f"./plots/{row.env_name}_hist_with_state_{row.sampling_policy}.png")
  print(file_path)
...
def state_cancer_hist(df):
  ...
  count, values = np.histogram(states_observed,
                               bins=np.linspace(0, 1, 5) * 1140)
  ...
  plt.savefig("./plots/state_cancer_hist.png")
  plt.savefig("./plots/state_cancer_hist.pdf")
  print("./plots/state_cancer_hist.png")
  plt.clf()
  print("")
  # import matplotlib.pyplot as plt
...
def plot_pendulum_state(df):
  ...
  for i, row in df.iterrows():
    ...
    if i <= 1:
      plt.plot(st[:, 0], st[:, 1], color="k",
               linewidth=2.0)  # , marker='o', alpha=.5,)
  ...
  plt.savefig("./plots/pendulum_state_obs_phase.png")
  plt.savefig("./plots/pendulum_state_obs_phase.pdf")
  print("./plots/pendulum_state_obs_phase.png")
  # plt.clf()

  h_obs, xedges_obs, yedged_obs = np.histogram2d(
      x=states_observed[:, 0],
      y=states_observed[:, 1],
      bins=(theta_bins, theta_dot_bins),
  )
  # np.histogram2d(x=all_states[:,0], y=all_states[:,1], bins=hist_bins)
  h_all, xedges_all, yedged_all = np.histogram2d(
      x=all_states[:, 0],
      y=all_states[:, 1],
      bins=(theta_bins, theta_dot_bins),
  )
  ...
  plt.imshow(
      ...
      cmap="Blues",
      interpolation="none",
      extent=[theta_min, theta_max, theta_dot_max, theta_dot_min],
      aspect="auto",
  )
  ...
  plt.xlabel(r"$\theta$")
  plt.ylabel(r"$\dot{\theta}$")
  plt.savefig("./plots/pendulum_state_obs_phase_hist.png")
  plt.savefig("./plots/pendulum_state_obs_phase_hist.pdf")
  print("./plots/pendulum_state_obs_phase_hist.png")
  plt.clf()
  ...
  plt.xlabel(r"$x$")
  plt.ylabel(r"$y$")
  plt.savefig("./plots/pendulum_state_obs_location.png")
  plt.savefig("./plots/pendulum_state_obs_location.pdf")
  print("./plots/pendulum_state_obs_location.png")
  # plt.clf()
  ...
  h_obs, xedges_obs, yedged_obs = np.histogram2d(x=x_obs,
                                                 y=y_obs,
                                                 bins=(x_bins, y_bins))
  # np.histogram2d(x=all_states[:,0], y=all_states[:,1], bins=hist_bins)
  h_all, xedges_all, yedged_all = np.histogram2d(x=x_all,
                                                 y=y_all,
                                                 bins=(x_bins, y_bins))
  h_obs_per_time = np.nan_to_num((h_obs / h_all))
  plt.imshow(
  ...
  plt.xlabel(r"$x$")
  plt.ylabel(r"$y$")
  plt.savefig("./plots/pendulum_state_obs_location_hist.png")
  plt.savefig("./plots/pendulum_state_obs_location_hist.pdf")
  print("./plots/pendulum_state_obs_location_hist.png")
  plt.clf()
  ...
  ax.bar(theta_mid_points, h_obs_per_time, width=theta_width, bottom=0.0)
  ax.set_xlabel("$\phi$")

  plt.savefig("./plots/pendulum_state_obs_location_hist_polar.png")
  plt.savefig("./plots/pendulum_state_obs_location_hist_polar.pdf")
  plt.clf()
  print("./plots/pendulum_state_obs_location_hist_polar.png")
  print("")
...
def gen_match_cp_obs_table(df, use_95_ci=True):
  ...
  aoc_with_normalization_reference_baselines = df[
      df.fixed_continuous_planning_observations.isnull()]

  if use_95_ci:
    df_out = (aoc_with_normalization_reference_baselines.groupby([
        "env_name",
        "sampling_policy",
        "model_name",
        "observing_var_threshold",
    ]).agg([np.mean, ci]).reset_index())
    cp_dif_obs = (cp_with_different_obs.groupby([
        "env_name",
        "sampling_policy",
        "model_name",
        "fixed_continuous_planning_observations",
    ]).agg([np.mean, ci]).reset_index())
  else:
    df_out = (aoc_with_normalization_reference_baselines.groupby([
        "env_name",
  ...
  for sampling_policy in df_out.sampling_policy.unique():
    ...
    for env_name in df_out.env_name.unique():
      ...
      if use_95_ci:
        line += (r"&" + f"{row.total_reward['mean'].iloc[0]:.{sf}g}" +
                 r"$\pm$" + f"{row.total_reward['ci'].iloc[0]:.{sf}g}" + r"&" +
                 f"{row.state_reward['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
                 f"{row.state_reward['ci'].iloc[0]:.{sf}g}" + r"&" +
                 f"{row.observations_taken['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
                 f"{row.observations_taken['ci'].iloc[0]:.{sf}g}")
      else:
        line += (r"&" + f"{row.total_reward['mean'].iloc[0]:.{sf}g}" +
                 r"$\pm$" + f"{row.total_reward['std'].iloc[0]:.{sf}g}" + r"&" +
                 f"{row.state_reward['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
                 f"{row.state_reward['std'].iloc[0]:.{sf}g}" + r"&" +
                 f"{row.observations_taken['mean'].iloc[0]:.{sf}g}" + r"$\pm$" +
                 f"{row.observations_taken['std'].iloc[0]:.{sf}g}")
  ...
  for i, row in cp_dif_obs.iterrows():
    ...
    for env_name in cp_dif_obs.env_name.unique():
      if use_95_ci:
        line += (r"&" + f"{row.total_reward['mean']:.{sf}g}" + r"$\pm$" +
                 f"{row.total_reward['ci']:.{sf}g}" + r"&" +
                 f"{row.state_reward['mean']:.{sf}g}" + r"$\pm$" +
                 f"{row.state_reward['ci']:.{sf}g}" + r"&" +
                 f"{row.observations_taken['mean']:.{sf}g}" + r"$\pm$" +
                 f"{row.observations_taken['ci']:.{sf}g}")
      else:
        line += (r"&" + f"{row.total_reward['mean']:.{sf}g}" + r"$\pm$" +
                 f"{row.total_reward['std']:.{sf}g}" + r"&" +
                 f"{row.state_reward['mean']:.{sf}g}" + r"$\pm$" +
                 f"{row.state_reward['std']:.{sf}g}" + r"&" +
                 f"{row.observations_taken['mean']:.{sf}g}" + r"$\pm$" +
...
def plot_match_cp_obs_plots(df):
  ...
  df_standard = (aoc_with_normalization_reference_baselines.groupby([
      "env_name",
      "sampling_policy",
      "model_name",
      "observing_var_threshold",
  ...
  for env_name in df_standard.env_name.unique():
    ...
    x = df_standard_t[df_standard_t.sampling_policy ==
                      "active_observing_control"][x_metric]["mean"]
    y_mean = df_standard_t[df_standard_t.sampling_policy ==
                           "active_observing_control"][y_metric]["mean"]
    y_std = df_standard_t[df_standard_t.sampling_policy ==
                          "active_observing_control"][y_metric]["std"]
    plt.plot(
        x,
        y_mean,
        "--o",
    ...
    x = cp_dif_obs_t[cp_dif_obs_t.sampling_policy ==
                     "continuous_planning"][x_metric]["mean"]
    print(x)
    y_mean = cp_dif_obs_t[cp_dif_obs_t.sampling_policy ==
                          "continuous_planning"][y_metric]["mean"]
    y_std = cp_dif_obs_t[cp_dif_obs_t.sampling_policy ==
                         "continuous_planning"][y_metric]["std"]
...
def cost_std_comparison(df):
  ...
  HEIGHT_SCALE = 0.30
  1 / HEIGHT_SCALE  # -(0.05 + LEGEND_Y_CORD)
  sn = configure_plotting_sn_params(sn,
                                    SCALE,
                                    HEIGHT_SCALE,
                                    use_autolayout=False)
  plt.gcf().subplots_adjust(bottom=0.40, left=0.2, top=0.95)

  sampling_policy_map = {
      "discrete_monitoring":
          r"Discrete Monitoring \quad\quad\quad $\mathcal{R}=85.7\pm0.526$",
      "discrete_planning":
          r"Discrete Planning",
      "continuous_planning":
          r"Continuous Planning",
      "active_observing_control":
          r"Active Sampling Control \quad $\mathcal{R}=98.8\pm0.174$",
      "random":
          "Random",
  }
  ...
  for i, row in df.iterrows():
    ...
    plt.plot(
        t[:min_length],
        costs_std[:min_length],
        label=sampling_policy_map[row.sampling_policy],
    )
...
def plot_telem_standard(result):
  ...
  result["observing_var_threshold"]
  ...
  plt.savefig(
      f"./plots/telem/telem_{env_name}_{sampling_policy}_{observation_noise}_{plot_seed}.png"
  )
  plt.savefig(
      f"./plots/telem/telem_{env_name}_{sampling_policy}_{observation_noise}_{plot_seed}.pdf"
  )
  plt.clf()
