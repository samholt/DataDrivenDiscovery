...
class TimeSeriesTransformer(nn.Module):
  ...
  def __init__(self, input_dim, feature_dim=250):
    ...
    self.linear_projection = nn.Linear(in_features=self.input_dim,
...
class TransAm(nn.Module):
  ...
  def __init__(
    ...
    self.src_mask = None
    self.input_transformer = TimeSeriesTransformer(input_dim=input_size,
...
class TransformerModel(nn.Module):
  ...
  def __init__(
    ...
    self.register_buffer("in_mean",
                         torch.tensor(states_actions_train.mean((0, 1))))
    self.register_buffer(
        "in_std",
        replace_zeros_with_ones(torch.tensor(states_actions_train.std((0, 1)))),
    )
    self.register_buffer(
        "output_mean",
...
class RNNModel(nn.Module):
  ...
  def __init__(
    ...
    self.gru = nn.GRU(obs_dim + action_dim,
                      hidden_dim,
                      batch_first=True,
                      num_layers=2)
