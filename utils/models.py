...
class TimeSeriesTransformer(nn.Module):
    def __init__(self, input_dim, feature_dim=250):
        ...
        self.linear_projection = nn.Linear(
            in_features=self.input_dim, out_features=self.feature_dim
        )
...
class TransAm(nn.Module):
    def __init__(
        ...
        self.src_mask = None
        self.input_transformer = TimeSeriesTransformer(
            input_dim=input_size, feature_dim=feature_size
        )
...
class TransformerModel(nn.Module):
    def __init__(
        ...
        self.model = TransAm(
            input_size=obs_dim + action_dim, output_size=obs_dim
        )

        self.register_buffer(
            "in_mean", torch.tensor(states_actions_train.mean((0, 1)))
        )
        self.register_buffer(
            "in_std",
            replace_zeros_with_ones(
                torch.tensor(states_actions_train.std((0, 1)))
            ),
        )
        self.register_buffer(
            "output_mean",
            torch.tensor(states_actions_train[:, :, :obs_dim].mean((0, 1))),
        )
        self.register_buffer(
            "output_std",
            replace_zeros_with_ones(
                torch.tensor(states_actions_train[:, :, :obs_dim].std((0, 1)))
            ),
        )
...
class RNNModel(nn.Module):
    def __init__(
        ...
        self.gru = nn.GRU(
            obs_dim + action_dim, hidden_dim, batch_first=True, num_layers=2
        )
        ...
        for m in self.stack.modules():
            if isinstance(m, nn.Linear):
                if model_initialization == "xavier":
                    ...
                    nn.init.normal_(
                        m.weight, mean=0, std=WEIGHTS_INITIALIZATION_STD
                    )
