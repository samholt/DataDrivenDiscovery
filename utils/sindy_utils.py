...
def convert_sindy_model_to_sympyjax_model(
    model, quantize=False, quantize_round_to=3
):
    feature_library_names = model.feature_library.get_feature_names().copy()
    coefs = model.coefficients()
    feature_names = model.feature_names
    return convert_sindy_model_to_sympyjax_model_core(
        feature_library_names,
        feature_names,
        coefs,
        quantize=quantize,
        quantize_round_to=quantize_round_to,
    )


def convert_sindy_model_to_sympyjax_model_core(
    feature_library_names,
    feature_names,
    coefs,
    quantize=False,
    ...
    feature_library_names = [
        fn.replace(" ", "*") for fn in feature_library_names
    ]
    fln_l = []
