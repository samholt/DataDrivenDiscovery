...
def get_llm_config(config, logger, name, rate_limiter):
  return deepcopy({
      ...
      "top_p":
          config.run.top_p,
      "frequency_penalty":
          config.run.frequency_penalty,
      "presence_penalty":
          config.run.presence_penalty,
      "stop":
          config.run.stop,
      "request_timeout":
          config.setup.api_request_timeout,
      "stream":
          config.setup.api_stream,
      "_open_ai_rate_limit_requests_per_minute":
          config.setup.open_ai_rate_limit_requests_per_minute,
      "_use_azure_api":
          config.setup.use_azure_api,
      "_logger":
          logger,
      "_name":
          name,
      ...
      "_retry_with_exponential_backoff__initial_delay":
          config.setup.api_retry_with_exponential_backoff__initial_delay,
      "_retry_with_exponential_backoff__exponential_base":
          config.setup.api_retry_with_exponential_backoff__exponential_base,
      "_retry_with_exponential_backoff__jitter":
          config.setup.api_retry_with_exponential_backoff__jitter,
      "_retry_with_exponential_backoff__max_retries":
          config.setup.api_retry_with_exponential_backoff__max_retries,
  })
...
def pretty_print_chat_messages(
  ...
  for msg in messages:
    role = msg["role"]
    color = COLORS.get(
        role, COLORS["system"])  # Default to system color if role not found
...
async def async_chat_completion_rl_inner(**kwargs):
  ...
  if use_azure_api:
    ...
    kwargs["_open_ai_rate_limit_requests_per_minute"] = model_details[
        "properties"]["rate_limit_(Requests per minute)"]
            ...
            if field == "type":
              ...
              if "properties" in v[field]:
                ...
                for NestedpropertiesKey in NestedParameters["properties"]:
                  function_tokens += len(
                      CL100K_ENCODER.encode(NestedpropertiesKey))
                  Nestedv = NestedParameters["properties"][NestedpropertiesKey]
                  for Nestedfield in Nestedv:
                    if Nestedfield == "type":
                      # function_tokens += 2
                      function_tokens += len(
                          CL100K_ENCODER.encode(Nestedv["type"]))
                    elif Nestedfield == "description":
                      # function_tokens += 2
                      function_tokens += len(
                          CL100K_ENCODER.encode(Nestedv["description"]))
