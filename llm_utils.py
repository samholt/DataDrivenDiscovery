...
OPENAI_MODEL_DETAILS_MAP = {
    "default": {"tpm": 250000, "rpm": 3000},
    "gpt-3.5-turbo": {"tpm": 90000, "rpm": 3500, "max_tokens": 4097},
    "gpt-3.5-turbo-0301": {"tpm": 90000, "rpm": 3500, "max_tokens": 4097},
    "gpt-3.5-turbo-0613": {"tpm": 90000, "rpm": 3500, "max_tokens": 4097},
    "gpt-3.5-turbo-16k": {"tpm": 180000, "rpm": 3500, "max_tokens": 16385},
    "gpt-3.5-turbo-16k-0613": {"tpm": 180000, "rpm": 3500, "max_tokens": 16385},
    "gpt-4": {"tpm": 40000, "rpm": 200, "max_tokens": 8192},
    "gpt-4-0314": {"tpm": 40000, "rpm": 200, "max_tokens": 8192},
    "gpt-4-0613": {"tpm": 40000, "rpm": 200, "max_tokens": 8192},
}
...
def setup_chat_rate_limiter(config: dict):
    if config.setup.use_azure_api:
        ...
        request_limit = model_details["properties"][
            "rate_limit_(Requests per minute)"
        ]
        token_limit = model_details["properties"][
            "rate_limit_(Tokens per minute)"
        ]
    else:
        model = config.run.model
...
def pretty_print_chat_messages(
    ...
    for msg in messages:
        ...
        color = COLORS.get(
            role, COLORS["system"]
        )  # Default to system color if role not found
        formatted_role = role.capitalize()
        content = msg["content"]
        if role == "assistant" and "function_call" in msg:
            formatted_role = "Function Call"
            print(
                f"{color}[{formatted_role}] [{msg['function_call']['name']}] {msg['function_call']['arguments']}\033[0m"
            )  # Reset color at the end
            if logger:
                logger.info(
                    f"[{formatted_role}] [{msg['function_call']['name']}] {msg['function_call']['arguments']}"
                )
        else:
            print(
                f"{color}[{formatted_role}] {content}\033[0m"
            )  # Reset color at the end
            if logger:
...
def chat_completion_rl(**kwargs):
    ...
    while True:
        ...
        # Retry on specified errors
            ...
            if use_azure_api:
                match = re.search(
                    r"Please retry after (\d+) seconds", e.args[0]
                )
                if match:
...
async def async_chat_completion_rl_inner(**kwargs):
    ...
    if use_azure_api:
        ...
        kwargs["_open_ai_rate_limit_requests_per_minute"] = model_details[
            "properties"
        ]["rate_limit_(Requests per minute)"]
    ...
    async for chunk in responses:
        ...
        chunk_message = chunk["choices"][0][
            "delta"
        ].to_dict_recursive()  # extract the message
...
def test_replace_newlines():
    ...
    assert replace_newlines(["No newline here", "Neither here"]) == [
        "No newline here",
        "Neither here",
    ]
...
def num_tokens_consumed_by_chat_request(
    messages, max_tokens=15, n=1, functions="", **kwargs
):
                ...
                for propertiesKey in parameters["properties"]:
                    ...
                    for field in v:
                        if field == "type":
                            function_tokens += 2
                            function_tokens += len(
                                CL100K_ENCODER.encode(v["type"])
                            )
                        elif field == "description":
                            function_tokens += 2
                            function_tokens += len(
                                CL100K_ENCODER.encode(v["description"])
                            )
                        elif field == "enum":
                            function_tokens -= 3
                            for o in v["enum"]:
                            ...
                            if "properties" in v[field]:
                                ...
                                for NestedpropertiesKey in NestedParameters[
                                    "properties"
                                ]:
                                    function_tokens += len(
                                        CL100K_ENCODER.encode(
                                            NestedpropertiesKey
                                        )
                                    )
                                    Nestedv = NestedParameters["properties"][
                                        NestedpropertiesKey
                                    ]
                            ...
                            print(
                                f"Warning: not supported field {field} : {v[field]}"
                            )
                function_tokens += 11
