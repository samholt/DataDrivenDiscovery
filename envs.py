...
class TestGeneratedEnvEnvironment(Environment):
    ...
    def evaluate_simulator_code(
        self, StateDifferential, config={}, logger=None
    ):
    ...
    def evaluate_simulator_code_on_test_dataset(
        self, StateDifferential, config={}, logger=None
    ):
        if self.env_name == "Cancer":
            ...
            gt_env = CancerEnv()
            env_state_diff_function_input = StateDifferential.replace(
                "np.", "jnp."
            )
    ...
    def execute_final_user_code(self, code_string):
        ...
        model, preprocessor = user_code_module.train_model(
            self.train_data.copy(), self.train_labels
        )
...
def train_model(X_train, y_train):
    ...
    return model, None"""
        ...
        self.assertAlmostEqual(
            reward, expected_accuracy, delta=0.01
...
"""
    ...
    def test_environment_monks_problems_2(self):
        ...
        env = DataScienceEnvironment(
            config, None, "data-science-monks-problems-2"
        )
        env.seed(42)
        env.reset()
